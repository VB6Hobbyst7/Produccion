...
If gOpeCGAdeudaMntPagaresMN = gsOpeCod Or gOpeCGAdeudaMntPagaresME = gsOpeCod Then
    ...
    If ChkContable.value = 0 Then
        If MsgBox("Este pagaré aun no tiene registro contable, ¿desea continuar con el proceso y generar los datos contables del pagaré?", vbYesNo + vbQuestion, "Aviso") = vbNo Then
            Exit Sub
        End If
    End If
Else
...
Private Sub cmdAjustar_Click()
    ...
    If nVal(Me.txtMontoEuros.Text) <> 0 Then
        ...
        Else
            MsgBox "Si es factor es igual a 1 no se realizará ningun ajuste.", vbExclamation, "Aviso"
        End If
    Else
