...
Begin VB.Form frmOpePagSunat 
   ...
   Begin VertMenu.VerticalMenu vFormPago 
      ...
      MenuItemsMax1   =   3
      MenuItemIcon11  =   "frmOpePagSUNAT.frx":030A
      MenuItemCaption11=   "Efectivo"
      MenuItemIcon12  =   "frmOpePagSUNAT.frx":0624
      MenuItemCaption12=   "Carta"
      MenuItemIcon13  =   "frmOpePagSUNAT.frx":093E
...
Else
    If lsDocTpo = TpoDocCheque Then
        ...
        'oDocPago.InicioCheque lsDocNRo, True, lsPersCod, gsOpeCod, lsPersNombre, gsOpeDesc, lsGlosa, lnImporte, gdFecSis, gsNomCmacRUC, lsSubCuentaIF, lsEntidadOrig, lsCtaEntidadOrig, lsDocVoucher, False, gsCodAge, Mid(txtBuscaEntidad, 18, 10)
        oDocPago.InicioCheque lsDocNRo, True, lsPersCod, gsOpeCod, lsPersNombre, gsOpeDesc, lsGlosa, lnImporte, gdFecSis, gsNomCmacRUC, lsSubCuentaIF, lsEntidadOrig, lsCtaEntidadOrig, lsDocVoucher, False, gsCodAge, Mid(txtBuscaEntidad, 18, 10), , lsTpoIf, lsPersCodIf, lsCtaBanco 'EJVG20121130
    End If
    ...
    Else
        Exit Sub
    End If
   If lsDocTpo = 47 Then ' Cheque
       lsCtaContHaber = oOpe.EmiteOpeCta(lsOpeCod, "H", "3", txtBuscaEntidad, ObjEntidadesFinancieras)
   Else ' Carta
       lsCtaContHaber = oOpe.EmiteOpeCta(lsOpeCod, "H", "4", txtBuscaEntidad, ObjEntidadesFinancieras)
   End If
End If
...
If MsgBox("Desea Grabar la Información", vbYesNo + vbQuestion, "Aviso") = vbYes Then
   ...
   lsMovNro = oNContFunc.GeneraMovNro(txtFecha, Right(gsCodAge, 2), gsCodUser)
   Set rsDebe = fgDetalle.GetRsNew
   
   'john
   If lsCtaITFD = "" Or lsCtaITFH = "" Then
      MsgBox "No existe Cuentas de ITF", vbInformation, "Aviso"
      Exit Sub
   End If
   '------
   If oCaja.GrabaPagoSunat(lsMovNro, lsOpeCod, txtMovDesc, Val(lsDocTpo), lsDocNRo, Format(lsFecha, gsFormatoFecha), rsBillete, rsMoneda, rsDebe, _
                           ...
                           txtDocNro, CDate(txtFechaFormula), lsDocVoucher, lsCtaITFD, lsCtaITFH, gnImpITF, IIf(chkAfectoITF.value = 1, True, False)) = 0 Then
      
      ImprimeAsientoContable lsMovNro, lsDocVoucher, lsDocTpo, lsDocumento, lbEfectivo, _
                              False, txtMovDesc, lsPersCodIf, lnImporte
      OK = True
      objPista.InsertarPista lsOpeCod, lsMovNro, gsCodPersUser, GetMaquinaUsuario, "1", "Pago SUNAT"
