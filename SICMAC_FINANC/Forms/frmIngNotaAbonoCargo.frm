...
Dim lsNroNota As String
Dim lnDocTpo As TpoDoc
Dim lnMonto As Currency
Dim lnMotivo As Long
Dim lsObjetoCodPadre As String
Dim lsObjetoCod As String
Dim lbOk As Boolean
Dim lnMotivoEsp As MotivoNotaAbonoCargo
...
If MsgBox("Desea Registrar " & lblTitulo & "???", vbYesNo + vbQuestion, "Aviso") = vbYes Then
    ...
    oDocRec.RegistroNotasAbonoCargo lnDocTpo, lsNroNotaNCNA, gNCNARegistrado, txtBuscarMotivo, CCur(txtImporte), lsMovNro, _
                                    txtBuscarObj, txtBuscarDet, Mid(gsOpeCod, 3, 1)
    
    lbOk = True
    lsNroNota = lsNroNotaNCNA
    lnMonto = CCur(txtImporte)
    lnMotivo = Int(txtBuscarMotivo)
...
End If
End Function
Public Sub Inicio(ByVal pnTpoDoc As TpoDoc, ByVal pnMonto As Currency, Optional ByVal pnMotivoEsp As MotivoNotaAbonoCargo = -1)
lnDocTpo = pnTpoDoc
lnMonto = pnMonto
