...
If nLin > 60 Then
   ...
   Linea lsTexto, PrnSet("C+") + CabeRepo(gsNomCmac, gsNomAge, "", "MONEDA " & IIf(gsSimbolo = gcME, "EXTRANJERA ", "NACIONAL"), Format(gdFecSis, gsFormatoFechaView), sTit, "( DEL " & txtFechaDel & " AL " & txtFechaAl & ")", "", "", P, 150) & PrnSet("C-"), 0
   Linea lsTexto, PrnSet("B+") + ImpreFormat("PROVEEDOR : " & Trim(lblNomPers), 50) & PrnSet("B-")
   nLin = 10
   If Me.Opt(0).value Then
        Linea lsTexto, PrnSet("C+") + PrnSet("B+") + IIf(Me.Chkproveedor.value = 0, String(200, "-"), String(155, "-"))
        'ALPA 20080919*****************************************
        ...
        Linea lsTexto, ImpreFormat("FECHA PAGO", 10) & ImpreFormat("PROVISION", 10) & ImpreFormat("EMISION", 10) & ImpreFormat("DOC. REFERENCIA", 20) & ImpreFormat("DOC. PAGO", 15) & ImpreFormat("DESCRIPCION", 40) & ImpreFormat("SUBTOTAL", 12) & ImpreFormat("TOTAL", 12) & IIf(Me.Chkproveedor.value = 0, ImpreFormat("PROVEEDOR", 30), "") & ImpreFormat("AGENCIAS", 8) 'By Capi 28122007 se adiciono agencia
        '*******************************************************
        Linea lsTexto, IIf(Me.Chkproveedor.value = 0, String(200, "-"), String(155, "-")) & PrnSet("B-")
   Else
        Linea lsTexto, PrnSet("C+") + PrnSet("B+") + IIf(Me.Chkproveedor.value = 0, String(200, "-"), String(155, "-"))
        'ALPA 20080919*****************************************
        ...
        Linea lsTexto, ImpreFormat("PROVISION", 10) & ImpreFormat("EMISION", 10) & ImpreFormat("TIPO", 6) & ImpreFormat("DOCUMENTO", 20) & ImpreFormat("DESCRIPCION", 40) & ImpreFormat("TOTAL", 12) & IIf(Me.Chkproveedor.value = 0, ImpreFormat("PROVEEDOR", 30), "") & ImpreFormat("AGENCIA", 4)
        '*******************************************************
        Linea lsTexto, IIf(Me.Chkproveedor.value = 0, String(200, "-"), String(155, "-")) & PrnSet("B-")
   End If
End If
...
lsCtaCont = Replace(lsCtaCont, "'", "")
sSQL1 = "exec stp_sel_ObtienePagoProveedores '" & lsCtaCont & "','" & Format(txtFechaDel, "yyyymmdd") & "','" & Format(txtFechaAl, "yyyymmdd") & "','" & IIf(Me.chkMontos.value = 1, "1", "0") & "'," & CDbl(Me.txtMontoIni.value) & "," & CDbl(Me.txtMontoFin.value) & ",'" & IIf(Me.Opt(0).value, "1", "0") & "','" & Trim(txtCodProv.Tag) & "'"
...
'Modificado PASI20150405
'Dim oCon As New DConecta
'oCon.AbreConexion
'Set rs = oCon.CargaRecordSet(sSQL1)
    If Not RSVacio(rs0) Then
        rs.MoveFirst
    End If
'END PASI
'End By
...
End If
RSClose rs
'oCon.CierraConexion Comentado PASI20150405
prg.Visible = False
End Sub
...
Private Sub cmdExportar_Click()
    ...
    lsNomHoja = "Hoja1"
    lsFile = "ReportePagoProveedores"
    
    '**************Se cambio de xls a xlsx NAGL Según INC1712260008
    lsArchivo = "\spooler\" & "PagoProveedor" & "_" & gsCodUser & "_" & Format(gdFecSis, "yyyymmdd") & "_" & Format(Time, "hhmmss") & ".xlsx"
    ...
    IniTablas = 3
    For i = 1 To fgProv.Rows - 1
        xlHoja1.Cells(IniTablas + i, 2) = fgProv.TextMatrix(i, 1)
        xlHoja1.Cells(IniTablas + i, 3) = fgProv.TextMatrix(i, 2)
        xlHoja1.Cells(IniTablas + i, 4) = fgProv.TextMatrix(i, 3)
        xlHoja1.Cells(IniTablas + i, 5) = fgProv.TextMatrix(i, 4)
        xlHoja1.Cells(IniTablas + i, 6) = fgProv.TextMatrix(i, 5)
        'xlHoja1.Cells(IniTablas + i, 7) = fgProv.TextMatrix(i, 6)
        xlHoja1.Cells(IniTablas + i, 7) = fgProv.TextMatrix(i, 8)
        xlHoja1.Cells(IniTablas + i, 8) = fgProv.TextMatrix(i, 9)
        xlHoja1.Cells(IniTablas + i, 9) = fgProv.TextMatrix(i, 10)
        xlHoja1.Cells(IniTablas + i, 10) = fgProv.TextMatrix(i, 11)
        xlHoja1.Cells(IniTablas + i, 11) = fgProv.TextMatrix(i, 12)
        'xlHoja1.Cells(IniTablas + i, 12) = fgProv.TextMatrix(i, 12)
    Next i
    
    xlHoja1.Range(xlHoja1.Cells(3, 2), xlHoja1.Cells(i + 2, 13)).Borders.LineStyle = 1
...
For N = 1 To Me.fgProv.Rows - 1
   ...
   If lOk Then
        ...
        If Me.Opt(0).value Then
            ...
            'ALPA 20080919*****************************************
            'Linea lsTexto, IIf(fgProv.TextMatrix(N, 10) = "", PrnSet("B-"), PrnSet("B+")) + ImpreFormat(fgProv.TextMatrix(N, 1), 10) & ImpreFormat(fgProv.TextMatrix(N, 2), 10) & ImpreFormat(fgProv.TextMatrix(N, 3), 10) & ImpreFormat(fgProv.TextMatrix(N, 4) & "-" & fgProv.TextMatrix(N, 5), 15) & ImpreFormat(fgProv.TextMatrix(N, 6) & IIf(fgProv.TextMatrix(N, 6) = "", "", "-") & fgProv.TextMatrix(N, 7), 15) & ImpreFormat(fgProv.TextMatrix(N, 8), 35) & ImpreFormat(CCur(IIf(fgProv.TextMatrix(N, 9) = "", "0", fgProv.TextMatrix(N, 9))), 10) & ImpreFormat(CCur(IIf(fgProv.TextMatrix(N, 10) = "", "0", fgProv.TextMatrix(N, 10))), 10) & IIf(Me.Chkproveedor.value = 0, ImpreFormat(fgProv.TextMatrix(N, 11), 35, 5), "") & ImpreFormat(fgProv.TextMatrix(N, 12), 25) & PrnSet("B-") 'By Capi 28122007
            Linea lsTexto, IIf(fgProv.TextMatrix(N, 10) = "", PrnSet("B-"), PrnSet("B+")) + ImpreFormat(fgProv.TextMatrix(N, 1), 10) & ImpreFormat(fgProv.TextMatrix(N, 2), 10) & ImpreFormat(fgProv.TextMatrix(N, 3), 10) & ImpreFormat(fgProv.TextMatrix(N, 4) & "-" & fgProv.TextMatrix(N, 5), 20) & ImpreFormat(fgProv.TextMatrix(N, 6) & IIf(fgProv.TextMatrix(N, 6) = "", "", "-") & fgProv.TextMatrix(N, 7), 15) & ImpreFormat(fgProv.TextMatrix(N, 8), 35) & ImpreFormat(CCur(IIf(fgProv.TextMatrix(N, 9) = "", "0", fgProv.TextMatrix(N, 9))), 10) & ImpreFormat(CCur(IIf(fgProv.TextMatrix(N, 10) = "", "0", fgProv.TextMatrix(N, 10))), 10) & IIf(Me.Chkproveedor.value = 0, ImpreFormat(fgProv.TextMatrix(N, 11), 35, 5), "") & ImpreFormat(fgProv.TextMatrix(N, 12), 25) & PrnSet("B-") 'By Capi 28122007
            '******************************************************
        Else
            'ALPA 20080919*****************************************
            ...
            Linea lsTexto, ImpreFormat(fgProv.TextMatrix(N, 2), 10) & ImpreFormat(fgProv.TextMatrix(N, 3), 10) & ImpreFormat(fgProv.TextMatrix(N, 4), 6) & ImpreFormat(fgProv.TextMatrix(N, 5), 20) & ImpreFormat(fgProv.TextMatrix(N, 8), 70) & ImpreFormat(CCur(IIf(fgProv.TextMatrix(N, 10) = "", "0", fgProv.TextMatrix(N, 10))), 12) & IIf(Me.Chkproveedor.value = 0, ImpreFormat(fgProv.TextMatrix(N, 11), 40, 5), "")
            '******************************************************
        End If
   End If
   nLin = nLin + 1
