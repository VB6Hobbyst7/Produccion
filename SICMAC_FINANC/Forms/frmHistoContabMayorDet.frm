...
Begin VB.Form frmHistoContabMayorDet 
   Caption         =   "Análisis de Cuentas Contables Historico"
   ClientHeight    =   7920
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   10230
   Icon            =   "frmHistoContabMayorDet.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   7920
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.ComboBox cboFiltro 
         ...
         EndProperty
         Height          =   315
         ItemData        =   "frmHistoContabMayorDet.frx":030A
         Left            =   7380
         List            =   "frmHistoContabMayorDet.frx":0320
   ...
   End
End
Attribute VB_Name = "frmHistoContabMayorDet"
...
'    Dim oCont As New nHistoBalanceCont 'NContAsientos
'    Dim oSdo  As New nHistoBalanceCont    'NCtasaldo
'    Set oSdo = Nothing
'    Set oImp = New nHistoBalanceCont
...
'    P = 0
'
'    Set rsCta = oCont.Histo_GetMayorCuenta(txtCtaCod, Format(txtFechaDel, "yyyymmdd"), Format(txtFechaAl, "yyyymmdd"), nVal(txtImporte), cboFiltro, , , IIf(Me.chkPersona.value = 1, True, False))
'
'    Set oCont = Nothing
'    If rsCta.EOF Then
'
...
'        Exit Sub
'    End If
'    Dim oCta  As New nHistoBalanceCont
'
'    Set rs = oCta.Histo_CargaCtaContClase(txtCtaCod)
'    Set oCta = Nothing
'    If Not rs.EOF Then
...
'    nSaldo = oSdo.Histo_GetCtaSaldo(rsCta!cCtaContCod, Format(CDate(txtFechaDel) - 1, gsFormatoFecha))
'    nSaldoIni = nSaldo
...
'Dim oBal As New nHistoBalanceCont
'
...
'End If
'
'Set rsCta = oBal.Histo_GetMayorCuenta(txtCtaCod.Text, Format(txtFechaDel, gsFormatoMovFecha), Format(txtFechaAl, gsFormatoMovFecha), nVal(txtImporte), cboFiltro, bME)
'
'Set oCont = Nothing
'fg.Rows = 2
'fg.Clear
...
'   fg.Rows = 2
'   fg.Row = 1
'   nSaldo = oBal.Histo_GetCtaSaldo(txtCtaCod, Format(CDate(txtFechaDel) - 1, gsFormatoFecha))
'   txtSaldoIni = PrnVal(nSaldo, 16, 2)
'   txtSaldoFin = PrnVal(nSaldo, 16, 2)
'
'   If bME = True Then
'      nSaldoME = oBal.Histo_GetCtaSaldo(txtCtaCod & "%", Format(CDate(txtFechaDel) - 1, gsFormatoFecha), False)
...
'   Exit Sub
'End If
'Set rs = oBal.Histo_CargaCtaContClase(txtCtaCod)
''Set oCta = Nothing
'If Not rs.EOF Then
'   sTipoCta = Trim(rs!cCtaCaracter)
'Else
...
'sCtaCod = txtCtaCod
'
'nSaldo = oBal.Histo_GetCtaSaldo(sCtaCod & "%", Format(CDate(txtFechaDel) - 1, gsFormatoFecha))
''John********
'If bME = True Then
'   nSaldoME = oBal.Histo_GetCtaSaldo(sCtaCod & "%", Format(CDate(txtFechaDel) - 1, gsFormatoFecha), False)
'End If
''---Fin
'txtSaldoIni = PrnVal(nSaldo, 16, 2)
''John*****
...
'Set oImp = New nHistoBalanceCont
'Me.Enabled = False
'sTexto = oImp.Histo_ImprimeMayorCta(txtCtaCod, txtCtaDesc, CDate(txtFechaDel), CDate(txtFechaAl), nVal(txtImporte), nVal(txtSaldoIniME), nVal(txtSaldoFinalME), cboFiltro, gnLinPage, IIf(Me.chkPersona.value = 1, True, False))
...
'txtCtaCod.rs = oCont.CargaCtaCont("", "Histo_CtaCont")
