...
Select Case Button.Key
    ...
    Case "imprimir"
        If lstArch.ListItems.Count <> 0 Then
            ...
            If Me.fraPagina.Enabled = False And nVal(txtPagIni) > 0 And nVal(txtPagFin) > 0 Then
                CargaRTFArchivo
                Exit Sub
            End If
            ...
            Dim lsImp  As String
            Dim nPos   As Integer
            Dim nPosAux As Currency
            Set fi = flCurrent.Files(lstArch.SelectedItem)
            rtfAux.Text = ""
            rtfAux.LoadFile fi.path, rtfText
            sTexto = rtfAux.Text
            
            Posi = InStr(1, sTexto, "T,66")
            If Posi = 0 Then
                Posi = InStr(1, sTexto, "F,66")
            End If
            frmImpresora.Show 1
            If lbCancela Then
                Exit Sub
            End If
            If Posi <> 0 Then
                parametros = Trim(Right(Trim(sTexto), 6))
                ExtraeParam
                tmpText = Mid(sTexto, 1, Posi - 1)
            Else
                tmpText = sTexto
            End If
            If chkPaginas.value = 1 Then
                If Posi <> 0 Then
                    tmpText = oImpresora.gPrnSaltoPagina & Mid(sTexto, 1, Posi - 1)
                Else
                    tmpText = oImpresora.gPrnSaltoPagina & sTexto
                End If
                X = 0: Y = 0
                vPosIni = 0: vPosFin = 0
                Do While True   'Ubicamos la Pagina Inicial
                    X = X + 1
                ...
                Else
                  vPosFin = vPosIni
                  Y = X
                  Do While True   'Ubicamos la Página Final
                      Y = Y + 1
                      nPosAux = InStr(vPosFin + 1, tmpText, oImpresora.gPrnSaltoPagina, vbTextCompare)
                      If nPosAux > 0 Then
                         vPosFin = nPosAux
                      Else
                         If Mid(tmpText, vPosFin + 1) <> "" And vPage = 0 Then
                            vPosFin = Len(tmpText)
                         End If
                         Exit Do
                      End If
            ...
            If lbCancela = False Then
                ...
                Next I
            End If
            MsgBox "Impresion Terminada Satisfactoriamente", vbInformation, "¡Aviso!"
        End If
    Case "salir"
            rtfView.Text = ""
            rtfAux.Text = ""
...
If lstArch.ListItems.Count <> 0 Then
    ...
    If UCase(Right(fi.Name, 3)) = "TXT" Or UCase(Right(fi.Name, 3)) = "RTF" Then
        ...
        rtfView.LoadFile fi.path, rtfText
        sTexto = rtfView.Text
        posicion = InStr(1, sTexto, "T,66")
        
        If posicion = 0 Then
            posicion = InStr(1, sTexto, "F,66")
            If posicion <> 0 Then
                sTexto = Mid(sTexto, 1, posicion - 1)
                parametros = Trim(Right(Trim(sTexto), 6))
                ExtraeParam
            End If
        Else
...
'        rtfView.Font = "Courier New"
    ...
    End If
    vRelle = String(Round(300000 / 155), "»") & oImpresora.gPrnSaltoLinea
    vPage = 0
    Dim sImpre As String
    Dim sPag   As String
    Dim sPagM  As String
    Dim N      As Currency
    Do While True And Len(Trim(sTexto)) > 0

        DoEvents
        vPage = vPage + 1
        N = InStr(1, sTexto, oImpresora.gPrnSaltoPagina)
        If N > 0 Then
           sPag = sPag & Mid(sTexto, 1, N - 1) & oImpresora.gPrnSaltoLinea & oImpresora.gPrnSaltoLinea & "<<Página :" & ImpreFormat(vPage, 7, 0) & ">>" & oImpresora.gPrnSaltoLinea & vRelle
           sTexto = Mid(sTexto, N + 1)
'            sTexto = Replace(sTexto, oImpresora.gPrnSaltoPagina, oImpresora.gPrnSaltoLinea & oImpresora.gPrnSaltoLinea & "<<Página :" & PrnVal(CCur(vPage), 7, 0) & ">>" & oImpresora.gPrnSaltoLinea & vRelle, 1, 1, vbTextCompare)
        Else
'            sTexto = sTexto & oImpresora.gPrnSaltoLinea & oImpresora.gPrnSaltoLinea & "<<Página :" & ImpreFormat(vPage, 7, 0) & ">>" & oImpresora.gPrnSaltoLinea
            sPag = sPag & sTexto & oImpresora.gPrnSaltoLinea & oImpresora.gPrnSaltoLinea & "<<Página :" & ImpreFormat(vPage, 7, 0) & ">>" & oImpresora.gPrnSaltoLinea
            Exit Do
        End If
        If vPage Mod 25 = 0 Then
            sPagM = sPagM & sPag
            sPag = ""
        End If
        If vPage Mod 300 = 0 Then
            sImpre = sImpre & sPagM
            sPagM = ""
        End If
    Loop
    rtfView.Text = sImpre & sPagM & sPag
    rtfView.SelStart = 0
    txtPagIni.Text = 1
...
Private Sub lstArch_ItemClick(ByVal Item As MSComctlLib.ListItem)
vPage = 0
End Sub
...
If KeyAscii = 13 Then
    If Val(txtPagFin.Text) <= vPage And Val(txtPagFin) >= Val(txtPagIni) And vPage > 0 Then
        cboPrevio.SetFocus
    End If
Else
...
If Not (Val(txtPagFin.Text) <= vPage And Val(txtPagFin) >= Val(txtPagIni)) And vPage > 0 Then
    Cancel = True
End If
End Sub
...
If KeyAscii = 13 Then
    If Val(txtPagIni.Text) >= 1 And Val(txtPagIni.Text) <= vPage And vPage > 0 Then
...
If Not (Val(txtPagIni.Text) >= 1 And Val(txtPagIni.Text) <= vPage) And vPage > 0 Then
    Cancel = True
End If
End Sub
