...
Begin VB.Form frmRepOpeConfirmadas 
   Caption         =   "Operaciones Confirmadas"
   ClientHeight    =   1545
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   5145
   Icon            =   "frmRepOpeConfirmadas.frx":0000
   LinkTopic       =   "Form1"
   ScaleHeight     =   1545
   ScaleWidth      =   5145
   ...
   End
End
Attribute VB_Name = "frmRepOpeConfirmadas"
...
Public Sub Inici(pnMoneda As Integer)
    lnMoneda = pnMoneda
    Me.Show 1
End Sub
...
Private Sub cmdGeneraReporte_Click()
    ...
    sSql = "ATM_ReporteOpConfirmadasConcialiadas '" & Format(txtFechaInicial.Text, "yyyymmdd") & "','" & Format(txtFechaFinal.Text, "yyyymmdd") & "', '" & Trim(Str(lnMoneda)) & "' "
    'MsgBox sSql
    sCadRep = "."
    ...
    sCadRep = sCadRep & Space(35) & "Reporte De Operaciones No Concilidas - Retencion entre " & Me.txtFechaInicial & " y " & Me.txtFechaFinal.Text & Chr(10) & Chr(10) & Chr(10)
    ...
    sCadRep = sCadRep & Space(5) & String(75, "-") & Chr(10)
    AbrirConexion
    R.Open sSql, C, adOpenStatic, adLockReadOnly, adCmdText
     
    Do While Not R.EOF
        lsItem = Format(lnCont, "#0000")
        lsTarjeta = R!PAN
        lsHora = Left(R!Hora, 2) & ":" & Mid(R!Hora, 3, 2) & ":" & Right(R!Hora, 2)
        lsDiaMes = Left(R!Fecha, 2) & "/" & Right(R!Fecha, 2)
