...
'Dim oSeguridad As New COMManejador.Pista
    ...
    Call oAcceso.CargarLogin_NEW(Trim(LblUsu.Caption), Trim(TxtClave.Text), bInterconexCorrecta, bClaveIncorrecta, rsITF, rsVar, psServerName, psDBName, psCadConexion, _
                              pMatMenuPer, bTieneAlgunPermiso, bTienePermisoRetiroSinFirma, bIniciarNuevoDia, gRsOpeF2, gRsExtornos, gRsOpeCMACRecep, gRsOpeCMACLlam, gRsOpeRepo, 6) 'marg20181217 agrego
    ...
    gsSistDescripcion = "SICMACM Recuperaciones" 'marg20181217 cambio
    gnSistCod = 6 'marg20181217 cambio
    gsFechaVersion = "20211020"             'CAMBIAR LA FECHA CADA VEZ QUE SE COMPILA"
    'AGREGADO POR ARLO 20170331
    
    If Not bInterconexCorrecta Then
        MsgBox "No se puede Establecer la Conexion con el Servidor" & Chr(10) & "Consulte con el Area de Sistemas", vbCritical, "Conexion SICMACM"
        'Set oAcceso = Nothing
        TxtClave.Text = ""
        TxtClave.SetFocus
    ...
    If Not bClaveIncorrecta Then
        MsgBox "Clave Incorrecta, Ingrese su Clave Nuevamente ", vbCritical, "Conexion SICMACM"
        'Set oAcceso = Nothing
        'AGREGADO POR ARLO 20170331
    ...
    MDISicmact.Caption = "Recuperaciones - " & Trim(gsNomAge) & " - " & Trim(gsCodUser) & Space(10) & gsServerName & "\" & gsDBName & Space(5) & Format(gdFecSis, "dd/mm/yyyy")
...
Dim sMaquinaActiva As String 'JUEZ 20160405
    ...
    If oSeguridad.ValidaAccesoPistaRF(gsCodUser) Then
        ...
        acceso = oSeguridad.ValidaIniciaSessionRF(gsCodPersUser, gdFecSis, GetMaquinaUsuario, 6, sMaquinaActiva) 'JUEZ 20160125
        If acceso Then
            MsgBox "No puede Acceder al Sistema porque tiene una Sesion Abierta en otra PC. Consulte con Help Desk", vbExclamation, "Ingreso SICMACM Recuperaciones"
            ...
            EnviarMail gsCorreoHost, gsCorreoEnvia, lsCorreoDestino, "Validación Acceso SICMACM Recuperaciones", lsContenido
            'Fin Envio Correo ****************************************************
