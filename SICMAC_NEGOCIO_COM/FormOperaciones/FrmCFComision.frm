VERSION 5.00
Begin VB.Form FrmCFComision 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Carta Fianza - Comisión "
   ClientHeight    =   4905
   ClientLeft      =   2265
   ClientTop       =   1845
   ClientWidth     =   7590
   Icon            =   "FrmCFComision.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form2"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   End
End
Attribute VB_Name = "FrmCFComision"
...
'*  ARCHIVO : frmCFComision
'*  CREACION: 15/09/2002      AUTOR :LAYG
'*  MODIFICACION
'***************************************************************************
'*  RESUMEN:  Permite registrar el cobro de comision de la Carta Fianza
'***************************************************************************
Option Explicit
...
Dim fsMovAutRen As String  'WIOR 20130313
Dim fbComisionRen As Boolean  'WIOR 20130313
...
Dim lsCta As String
    lsCta = frmCFPersEstado.inicio(Array(gColocEstAprob), "Comision de Carta Fianza", Array(gColCFComercial, gColCFPYME, gColCFTpoProducto), , True) 'WIOR 20130313 QUITO gColocEstRenovada
...
ActXCodCta.Enabled = False
    ...
    If Not R.BOF And Not R.EOF Then
        ...
        Set rsCFRenNew = oCFRenNew.ObternerAutRenovacion(psCta, "0")
        
        If Not (rsCFRenNew.EOF And rsCFRenNew.BOF) Then
            If rsCFRenNew.RecordCount > 0 Then
                ldNEmision = CDate(rsCFRenNew!NEmision)
                ldNVenc = CDate(rsCFRenNew!NVencimiento)
                lnMontoR = CDbl(rsCFRenNew!nMonto)
                lsDescEstado = "Renovado"
                fsMovAutRen = Trim(rsCFRenNew!cMovAut)
                fbComisionRen = True
            Else
                ldNEmision = R!dAsignacion
                ldNVenc = CDate(IIf(IsNull(R!dVencApr), "", Format(R!dVencApr, "dd/mm/yyyy")))
                lnMontoR = CDbl(IIf(IsNull(R!nMontoApr), "", Format(R!nMontoApr, "#0,000.00")))
                lsDescEstado = fgEstadoColCFDesc(R!nPrdEstado)
                fsMovAutRen = ""
                fbComisionRen = False
            End If
        Else
        ...
        'lblEstado.Caption = fgEstadoColCFDesc(R!nPrdEstado) 'WIOR 20130313 COMENTO
        lblEstado.Caption = lsDescEstado 'WIOR 20130313
        'ldFechaAsi = R!dAsignacion'WIOR 20130313 COMENTO
        ldFechaAsi = ldNEmision 'WIOR 20130313
        ...
        lblMontoApr = IIf(IsNull(lnMontoR), "", Format(lnMontoR, "#0,000.00"))
        'lblFecVencCF = IIf(IsNull(R!dVencApr), "", Format(R!dVencApr, "dd/mm/yyyy"))
        lblFecVencCF = IIf(IsNull(ldNVenc), "", Format(ldNVenc, "dd/mm/yyyy"))
        ...
        Set loCFCalculo = New COMNCartaFianza.NCOMCartaFianzaCalculos
            If fbComisionTrimestral = False Then ' Caja Trujillo
                'lnComisionCalculada = Format(loCFCalculo.nCalculaComisionCF(R!nMontoApr, DateDiff("d", ldFechaAsi, R!dVencApr), fpComision, CInt(Mid(Trim(psCta), 9, 1))), "#,##0.00")'WIOR 20130313 COMENTO
                lnComisionCalculada = Format(loCFCalculo.nCalculaComisionCF(lnMontoR, DateDiff("d", ldFechaAsi, ldNVenc), fpComision, CInt(Mid(Trim(psCta), 9, 1))), "#,##0.00") 'WIOR 20130313
            Else  ' Caja Metropolitana
            '*** PEAC 20090713 se agrego los parametros  psCta, 6
                'lnComisionCalculada = Format(loCFCalculo.nCalculaComisionTrimestralCF(R!nMontoApr, DateDiff("d", ldFechaAsi, R!dVencApr), R!nModalidad, Mid(R!cCtaCod, 9, 1), psCta, 6), "#,###0.00")'WIOR 20130313 COMENTO
                lnComisionCalculada = Format(loCFCalculo.nCalculaComisionTrimestralCF(lnMontoR, DateDiff("d", ldFechaAsi, ldNVenc), R!nModalidad, Mid(R!cCtaCod, 9, 1), psCta, 6), "#,###0.00") 'WIOR 20130313
            End If
        Set loCFCalculo = Nothing
        If lnComisionPagada >= lnComisionCalculada Then
            MsgBox " Ya se realizo el Pago de Comisión para la Carta Fianza", vbInformation, "Aviso"
            LimpiarControles
            Exit Sub
        Else
        ...
        'If R!dVencApr < gdFecSis Then 'WIOR 20130313 COMENTO
        If ldNVenc < gdFecSis Then 'WIOR 20130313
            MsgBox "Fecha de Vencimiento Aprobada es anterior a la Fecha Actual ", vbInformation, "Aviso"
        ...
        If gITF.gbITFAplica = True Then
           If Not gITF.gbITFAsumidocreditos Then
                ...
                '*** BRGO 20110908 ************************************************
                nRedondeoITF = fgDiferenciaRedondeoITF(CCur(Me.TxtITF))
                If nRedondeoITF > 0 Then
                   Me.TxtITF = Format(CCur(Me.TxtITF) - nRedondeoITF, "#,##0.00")
                End If
                '*** END BRGO
...
Dim lnMovNro As Long ' BRGO 20110908
...
If MsgBox("Desea Grabar Cobranza de Comision de Carta Fianza", vbInformation + vbYesNo, "Aviso") = vbYes Then
    'lnUltTrans = ObtieneUltimaTransCartaFianza(vCodCta, dbCmact) + 1
    'Genera el Mov Nro
    ...
    Call loNCartaFianza.nCFComision(vCodCta, lsFechaHoraGrab, lsMovNro, lnMonto, gITF.gbITFAplica, gITF.gbITFAsumidocreditos, CCur(Val(Me.TxtITF)), , fbComisionRen, fsMovAutRen) 'WIOR 20130313 AGERGO fbComisionRen, fsMovAutRen
    ...
    ' *** Impresion
        Set loImprime = New COMNCartaFianza.NCOMCartaFianzaImpre
            lsCadImprimir = loImprime.nPrintReciboCFComision(gsNomAge, lsFechaHoraGrab, vCodCta, lblNombre, _
                    lblNomAcreedor, Me.lblMontoApr, lblFecVencCF, lnMonto, gsCodUser, "", gsCodCMAC, CDbl(Val(TxtITF.Text)), gImpresora, gbImpTMU)
...
Set rsPersona = oDPersona.ObtenerUltimaVisita(Trim(lblcodigo.Caption))
...
If rsPersona.RecordCount > 0 Then
    If Not (rsPersona.EOF And rsPersona.BOF) Then
        If Trim(rsPersona!sUsual) = "3" Then
            ...
            Call frmPersona.inicio(Trim(lblcodigo.Caption), PersonaActualiza)
        End If
    End If
End If
'WIOR FIN ***************************************************************
