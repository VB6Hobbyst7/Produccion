...
Dim nNumRegTmp As Integer ' CTI2 20181012
...
Dim i As Integer
nNumRegTmp = 0 'CTI2 20181012
'CTI2 20181217 Comentado
...
On Error GoTo error_handler
    ...
    dlgArchivo.Filter = "Archivos Convenio (*.cobros)|*.cobros|Archivos Corresponsalia (*.pagos)|*.pagos"
    dlgArchivo.ShowOpen
    If dlgArchivo.FileName <> Empty Then
        fsPathFile = dlgArchivo.FileName
        fsRuta = fsPathFile
                If fsPathFile <> Empty Then
                    For i = Len(fsPathFile) - 1 To 1 Step -1
                            If Mid(fsPathFile, i, 1) = "\" Then
                                fsPathFile = Mid(dlgArchivo.FileName, 1, i)
                                fsNomFile = Mid(dlgArchivo.FileName, i + 1, Len(dlgArchivo.FileName) - i)
                                Exit For
                            End If
                     Next i
                  Screen.MousePointer = 11
            ...
            Screen.MousePointer = 0
    Else
        MsgBox "No se seleccionó un Archivo", vbExclamation, "Aviso"
        Exit Sub
    End If
...
End With
           ...
           If per Then
                ...
                Else
                  'CTI2 20181011 *********
                  If rsx!nImpToTS = rsx!cantRegistros Then
                    MsgBox "El archivo ya fue Procesado con Anterioridad, Verifique!!!", vbInformation, "Aviso"
                    LlenaRecordSet_Datos54 = False
                  Else
                    MsgBox "El archivo ya fue Procesado con Anterioridad, Se procesarán los pagos pendientes", vbInformation, "Aviso"
                    LlenaRecordSet_Datos54 = True
                  End If
                  'END CTI2 20181011 ******
                End If
...
End With
           ...
           If per Then
                ...
                Else
                  'CTI2 20181012 **********
                  If rsx!nNumReg = rsx!cantRegistros Then
                    MsgBox "El archivo ya fue Procesado con Anterioridad, Verifique!!!", vbInformation, "Aviso"
                    LlenaRecordSet_Datos60 = False
                  Else
                    nNumRegTmp = rsx!cantRegistros
                    cod_det = rsx("id") 'obtener el id de la cabecera
                    MsgBox "El archivo ya fue Procesado con Anterioridad, Se procesarán los pagos pendientes", vbInformation, "Aviso"
                    LlenaRecordSet_Datos60 = True
                  End If
...
End With
       ...
       If Not rsx.EOF And Not rsx.BOF Then
            ...
            'CTI2 20181011 *****
            Dim ConnBase As COMDCredito.DCOMCredActBD
            Dim sError As String
            Set ConnBase = New COMDCredito.DCOMCredActBD
            ConnBase.dBeginTrans
            'CTI2 END **********
      
            'Call oCredD.Insertar_Datos_Detalle_BN(cod_det, rsDocs) 'inserto en detalle CTI2 20181121
            Call ConnBase.Insertar_Datos_Detalle_BN(cod_det, rsDocs) 'inserto en detalle CTI2 20181121
            
            'para actualizar los Mov por cada operacion
            'Set rsy = oGen.DevolverIDDatos_Detalle(cod_det, CInt(Datos4.nNroCta), psCtaCod, Datos4.dFechaCobro) CTI2 20181121
            Set rsy = ConnBase.DevolverIDDatos_Detalle(cod_det, CInt(Datos4.nNroCta), psCtaCod, Datos4.dFechaCobro) 'CTI2 20181121
            ...
            If Not CargaDatos(cod_dety, psCtaCod, CDbl(rsDocs.Fields("nImporteCobrado")), Datos4.dFechaCobro, CInt(Datos4.nNroCta), bCobraComision, Cobros, ConnBase) Then
                   Dim ultm As Integer
                   n_procesados = n_procesados + 1
                   'Call oCredD.Actualizar_DetallexEst_BN(cod_det, psCtaCod, Datos4.nNroCta, (Datos4.dFechaCobro), 1) CTI2 20181121
                   ConnBase.dRollbackTrans 'ADD CTI2 20181121
...
Private Function CargaDatos(ByVal cod_det As Double, ByVal psCtaCod As String, _
                            pnImporteCobrado As Double, pdFechaCobro As String, _
                            pnNroCta As Integer, Optional valor As Boolean = False, _
                            Optional ByVal pTpoPago As TipoPagoBcoNac = Corresponsalia, _
                            Optional ByVal pTrans As COMDCredito.DCOMCredActBD = Nothing) As Boolean
'CTI2 20181122 ADD pTrans
...
Dim tmpFecha As Date 'GEMO-TORE 20201125
'Corresponsalia
Dim Cuota As Double
...
Dim ultm As Integer
    ...
    'GEMO-TORE 20201125
     tmpFecha = CDate(Mid(pdFechaCobro, 1, 4) & "-" & Mid(pdFechaCobro, 5, 2) & "-" & Mid(pdFechaCobro, 7, 2))
     nDiasAtras = CInt(DateDiff("d", tmpFecha, gdFecSis))
    'END GEMO-TORE 20201125
    ...
    If nDiasAtras > 0 Then
        ...
        Perdonar_Mora_ICV psCtaCod, nDiasAtras, CDate(Mid(pdFechaCobro, 1, 4) & "-" & Mid(pdFechaCobro, 5, 2) & "-" & Mid(pdFechaCobro, 7, 2))
    End If
    ...
    If Not rsCredVig.BOF And Not rsCredVig.EOF Then
        ...
        If bValorProceso Then
               ...
               'CTI2 20190415
               'Se está enviando un valor al parámetro que normalmente era vacio
               'Esto para corregir un incidente en la opción de impresión de voucher dentro del método "GrabarPagoCuotas"
               Dim pnCancelado(4) As Integer
               pnCancelado(3) = 0  ' Se coloca el valor 0 porque es el valor por defecto que va si el parámetro es opcional dentro del método "ImprimeBoleta" de COMNCredito.NCOMCredDoc
               'CTI2 END
               ...
               Call oCredito.GrabarPagoCuotas(psCtaCod, nMiVivienda, nCalPago, CDbl(MontoPagosinITF), _
                                ...
                                sError, "", sImpreBoleta, LblDiasAtraso, gsProyectoActual, gbImpTMU, , "", "", "", "", NroMov, , , , , pTrans, pnCancelado, nDiasAtras)
               ...
               'Operacion de egreso
                ...
                'ADD CTI2 20181122 *******
                sMovNro = pTrans.GeneraMovNro(gdFecSis, gsCodAge, gsCodUser)
                pTrans.OtrasOpeDepCtaBanco sMovNro, "01.1090100824631.0110001", psCtaCod, MontoPagosinITF + nITF, cNomCliente, gsNomAge, IIf(LblMoneda = "SOLES", 1, 2), 300593
                nMovNroR = pTrans.GetnMovNro(sMovNro)
                lsMovNro = pTrans.GeneraMovNro(gdFecSis, gsCodAge, gsCodUser)
                'END CTI2 ****************
              ...
              If pTpoPago = Corresponsalia Then 'EJVG20131128
                If LblMoneda = "SOLES" Then
                    cod_ageContable = "2918070101" & gsCodAge
                     ...
                     'oCredD.GrabaDeposiBancoNegocioFinan lsMovNro, "401583", psMovDesc, "1113010201",
                    pTrans.GrabaDeposiBancoNegocioFinan lsMovNro, "401583", psMovDesc, "1113010201", _
                ...
                Else
                     ...
                     'oCredD.GrabaDeposiBancoNegocioFinan lsMovNro, "402583", psMovDesc, "1123010201",
                    pTrans.GrabaDeposiBancoNegocioFinan lsMovNro, "402583", psMovDesc, "1123010201", _
                    cod_ageContable, CCur(MontoPagosinITF + nITF), 0, "", pdFechaCobro, "", _
              ...
              End If
              'FIN ORCR20140714--------------------------------------------------------------
              'nMovNroC = clsMov1.GetnMovNro(lsMovNro) CTI2 20181122 COMENTADO
              nMovNroC = pTrans.GetnMovNro(lsMovNro)
              'Set clsMov1 = Nothing

              'Guardar Movs (pago, reversion y contable) --------------------------------------
                'If NroMov > 0 And nMovNroR > 0 Then CTI2 COMENTADO
                If NroMov > 0 And nMovNroR > 0 And nMovNroC > 0 Then
                    'If valor Then
                    If pTpoPago = Corresponsalia Then 'EJVG20131128
                        Call pTrans.Actualizar_DetallexEst_MovCorresponsalia(cod_det, NroMov, nMovNroR, nMovNroC, comision)
                    ElseIf pTpoPago = Cobros Then
                        Call pTrans.Actualizar_DetallexEst_Mov(cod_det, NroMov, nMovNroR, nMovNroC)
                    End If
                    If est = True Then
                        If (dif - nITF) > 0 Then
                            'If valor Then
                            If pTpoPago = Corresponsalia Then 'EJVG20131128
                                Call pTrans.Insertar_Datos_SobrantesCorresponsalia(psCtaCod, nNroCta, dif - nITF, FechaCobro1, cNomCliente, NroMov)
                            ElseIf pTpoPago = Cobros Then
                                Call pTrans.Insertar_Datos_Sobrantes(psCtaCod, nNroCta, dif - nITF, FechaCobro1, cNomCliente, NroMov)
                            End If
                        End If
                    End If
                Else
              ...
              ElseIf ultm = p2 And n_procesados > 0 Then
                    MsgBox "El Pago Finalizó correctamente pero hubieron registros NO procesados, vuelva a cargar la trama!!"
                    Me.cmdLlenar.Enabled = True
                    Me.CmdCargaArch.Enabled = False
               ElseIf ultm = p2 Then
...
Public Sub Leer_Lineas(strTextFile As String)
    ...
    Set rsx = New ADODB.Recordset
    Set oGen = New COMDCredito.DCOMCreditos
    Dim ultm As Integer 'CTI2 20181011
    Dim nImporteCuota As Currency 'CTI2 20181011
            ...
            nCerrar = 0
             ...
             End If
                   ...
                   If Not rsx.EOF And Not rsx.BOF Then
                       ...
                       MsgBox "Se van a procesar " & (RegCan - rsx!cantRegistros) & " registros, Esta operación puede demorar unos minutos ...", vbInformation, "Aviso" 'CTI2 ADD rsX!cantRegistros
                   Else
        ...
        Else
            If Len(str_Linea) = 234 Then
                ...
                With Datos4
                            ...
                            nImporteCuota = Format(Mid(.nImporteCuota, 1, Len(.nImporteCuota) - 2), ".") + Right(.nImporteCuota, 2)
                            If Not oGen.VerificaItemTramaBcoNac(fd, .cCodCta, .nNroCta, nImporteCuota) Then
...
Public Sub Leer_Lineas1(strTextFile As String)
    ...
    Set oGen = New COMDCredito.DCOMCreditos
    Dim ultm As Integer 'CTI2 20181011
    Dim nImporteCobrado As Currency 'CTI2 20181011
    ...
    Else
        ...
        Do 'grabar por cada item el cuerpo
            ...
            lineas = lineas + 1
            If lineas = 1 Then
               MsgBox "Se van a procesar " & (Tot_Corr - 1 - nNumRegTmp) & " registros, Esta operación puede demorar unos minutos ...", vbInformation, "Aviso"
               oBarra.ShowForm Me
               oBarra.CaptionSyle = eCap_CaptionPercent
               oBarra.Max = Tot_Corr
           ...
           End If
            ...
            If Not (Tot_Corr = lineas) Then
                If Mid(str_Linea, 1, 4) = "0085" And lineas <> Tot_Corr Then
                   ...
                   nImporteCobrado = CDbl((Datos6.nNumPag) + "." + (Datos6.nNumDec))
                                      
                    If Not oGen.VerificaItemTramaBcoNacCorresponsalia(fd, Datos6.cCodCta, Datos6.nNroCuota, nImporteCobrado) Then
                        Call LlenaRecordSet_Datos236 'cuerpo
                    Else
                        procesados = procesados + 1
                        ultm = n_procesados + procesados
                        If procesados = p2 Then
                             MsgBox "El Pago Finalizó correctamente"
                             Me.cmdLlenar.Enabled = True
                             Me.CmdCargaArch.Enabled = False
                        ElseIf ultm = p2 And n_procesados > 0 Then
                             MsgBox "El Pago Finalizó correctamente pero hubieron registros NO procesados, Vuelva a cargar la trama!!"
                             ...
                             Me.CmdCargaArch.Enabled = False
                        ElseIf ultm = p2 Then
                             MsgBox "El Proceso de Pago Finalizó correctamente"
                             Me.cmdLlenar.Enabled = True
                             Me.CmdCargaArch.Enabled = False
                        End If
                    End If
...
End With
       ...
       If Not rsx.EOF And Not rsx.BOF Then
            ...
            'Call oCredD.Insertar_Datos_Detalle_corresponsaliaBN(cod_det, rsDocs)  'inserto en detalle, CTI2 20181122 COMENTADO
            Call ConnBase.Insertar_Datos_Detalle_corresponsaliaBN(cod_det, rsDocs)  'inserto en detalle CTI2 ADD
            'para actualizar los Mov por cada operacion
            'Set rsy = oGen.DevolverIDDatos_DetalleCorresponsalia(cod_det, CInt(Datos6.nNroCuota), psCtaCod, Datos6.dProceso) CTI2 20181122 COMENT
                        
            Set rsy = ConnBase.DevolverIDDatos_DetalleCorresponsalia(cod_det, CInt(Datos6.nNroCuota), psCtaCod, Datos6.dProceso) 'CTI2 20181122 ADD
            ...
            'se cambio por Importe Cobrado, debido a pagos parciales
            If Not CargaDatos(cod_dety, psCtaCod, CDbl((Datos6.nNumPag) + "." + (Datos6.nNumDec)), Datos6.dProceso, CInt(Datos6.nNroCuota), True, Corresponsalia, ConnBase) Then
                   Dim ultm As Integer
                   n_procesados = n_procesados + 1
                   'Call oCredD.Actualizar_DetallexEst_BN(cod_det, psCtaCod, CInt(Datos6.nNroCuota), (Datos6.dProceso), 0, 0)
                   ConnBase.dRollbackTrans 'ADD CTI2 20181122
                   ultm = n_procesados + procesados
                      If n_procesados = p2 Then
...
'RIRO 20210513 ****************
Sub Perdonar_Mora_ICV(cCtaCod As String, nNumPerdon As Integer, dFProceso As Date)
    Dim oCale As COMDCredito.DCOMCredActBD
    Dim dFechaCalc As Date
    If nNumPerdon > 0 Then
        dFechaCalc = DateAdd("d", -1, gdFecSis)
        dFechaCalc = DateAdd("d", -nNumPerdon, dFechaCalc)
        Set oCale = New COMDCredito.DCOMCredActBD
        Call oCale.PerdonarMoraICV(cCtaCod, dFechaCalc, gsCodUser)
    End If
End Sub
'END RIRO *********************
