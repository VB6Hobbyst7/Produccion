...
Dim nMontoPrimaActual As Double 'CTI2 FERIMORO ERS034-2019
Dim nPagosPendientes As Integer 'CTI2 FERIMORO ERS034-2019
...
Private Sub cmbBuscar_Click()
    ...
    If lsPersCod <> "" Then
        ...
        If Not (rsDatos.EOF And rsDatos.BOF) Then
            ...
            If rsDatos!factor = 0 Then
                MsgBox "Cliente Registra Seguro Desafiliado por Falta de Pago ", vbInformation, "Alerta SICMACM"
                nPagosPendientes = rsDatos!factor
                Call LimpiaFormulario
                Exit Sub
            Else
               lblMonto.Text = Format(rsDatos!nMontoPrimaVencido, gsFormatoNumeroView)
               nPagosPendientes = rsDatos!factor
            End If
            nMontoPrimaActual = rsDatos!nMontoPrima
            '-----------------------------------------
            lblMoneda.Caption = "S/."
...
Private Sub Grabar()
    ...
    On Error GoTo Error
    'CTI2 FERIMORO ERS034-2019 (03112020)
    'lnMonto = lblMonto.Text
    lnMonto = nMontoPrimaActual
    '---------------------------------------
    ...
    Dim mens1, mens2 As String
    Dim nBucle As Integer

    If lbPrimaAnual Then
            clsCap.veriCroSegSepAnual lsDocumento, gdFecSis
            Mensaje = clsCapMov.cnsVeriPagoAnualSeguro(lsDocumento, gdFecSis)
            If Mensaje <> "" Then
               lineas = Split(Mensaje, "%")
               mens1 = lineas(0)
               mens2 = lineas(1)
               MsgBox mens1 & vbCrLf & vbCrLf & mens2, vbInformation, "Aviso"
               Call LimpiaFormulario
               Exit Sub
            End If
    End If
    '-------------------------------------------------------------
    'CTI2 FERIMORO : ERS034-2019   26082020
    'nPagosPendientes = clsCapMov.cnsVerificaTiempoGracia(lsDocumento, gdFecSis)
    If nPagosPendientes = 2 Then
        If MsgBox("Cliente Registra Dos Primas Pendientes " & vbCrLf & vbCrLf & "Monto a Pagar " & lblMoneda.Caption & " " & Format((CDbl(lnMonto) * CDbl(nPagosPendientes)), gsFormatoNumeroView) & vbCrLf & vbCrLf & "Desea Continuar?", vbQuestion + vbYesNo, "Aviso") = vbNo Then
           Call LimpiaFormulario
           Exit Sub
        End If
    Else
      If nPagosPendientes = 0 Then
          MsgBox "Cliente Registra Seguro Desafiliado por Falta de Pago ", vbInformation, "Alerta SICMACM"
          Call LimpiaFormulario
          Exit Sub
      End If
...
'        If nPagosPendientes = 3 Then
'          MsgBox "Cliente Registra Seguro Desafiliado por Falta de Pago ", vbInformation, "Alerta SICMACM"
'          Call LimpiaFormulario
'          Exit Sub
'        Else
'            If nPagosPendientes = -1 Then
'              MsgBox "Cliente ya registra pago del mes", vbInformation, "Alerta SICMACM"
'              Call LimpiaFormulario
'              Exit Sub
'            Else
...
'        End If
    ...
    Dim nExSegCronograma As Integer
    ...
    Do
          ...
          sFecAfiliacion = clsCapMov.cnsSeguroCronograma(lsDocumento)
          If sFecAfiliacion = "" Then
              sFecAfiliacion = IIf(lbPrimaAnual, DateAdd("M", 12, lsFecAfiliacion), DateAdd("M", 1, lsFecAfiliacion))
              nExSegCronograma = 1
          End If
        ...
        'Call oSeg.ActualizaEstadoSeguroSepelio(lsDocumento, sFecAfiliacion, lnMovNro, 501) 'APRI20171027 ERS028-2017 CAMBIÒ ESTADO 1 POR 501
          Call oSeg.ActualizaSegurosCronograma(1, sFecAfiliacion, IIf(lbPrimaAnual, 1, 0), lnMovNro, lsDocumento, nExSegCronograma) ''CTI2 FERIMORO ERS034-2019 25082020 : COMENTADO
          
          sMesb = "SEG. POLIZA " & MostrarMesPago(sFecAfiliacion)
          rsPagos.AddNew _
            Array("mes", "monto", "nPrima"), _
            Array(sMesb, "S/. " & CStr(Format(lnMonto, "#,##0.00")), lnMonto)
          rsPagos.MoveFirst
    ...
    nBucle = nBucle + 1
    Loop While nBucle < nPagosPendientes
    ...
    MsgBox "Debe realizar el cobro de " & lblMoneda.Caption & " " & Format((CDbl(lnMonto) * CDbl(nPagosPendientes)), gsFormatoNumeroView), vbInformation, "Aviso"
    
    nProxFechaB = clsCapMov.cnsSeguroCronograma(lsDocumento)
    
    lsBoleta = oSeg.ImprimeBoletaAfilicacionSeguroSepelio(lnMovNro, lsMov, gsNomAge, gbImpTMU, , nProxFechaB, rsPagos)
    
'comentado por CTI2 FERIMORO: ERS034-2019  27082020
'lsMov = clsCont.GeneraMovNro(gdFecSis, gsCodAge, gsCodUser)
...
End Sub
'CTI2 FERIMORO : ERS034-2019   26082020
Public Function MostrarMesPago(ByVal sFecha As Date) As String
Dim sMes, sAnno, bResultado As String

sMes = Format(Month(sFecha), "00")
sAnno = Format(sFecha, "YY")

Select Case sMes
 Case "01": bResultado = "ENE" & sAnno
 Case "02": bResultado = "FEB" & sAnno
 Case "03": bResultado = "MAR" & sAnno
 Case "04": bResultado = "ABR" & sAnno
 Case "05": bResultado = "MAY" & sAnno
 Case "06": bResultado = "JUN" & sAnno
 Case "07": bResultado = "JUL" & sAnno
 Case "08": bResultado = "AGO" & sAnno
 Case "09": bResultado = "SET" & sAnno
 Case "10": bResultado = "OCT" & sAnno
 Case "11": bResultado = "NOV" & sAnno
 Case "12": bResultado = "DIC" & sAnno
End Select

MostrarMesPago = bResultado
