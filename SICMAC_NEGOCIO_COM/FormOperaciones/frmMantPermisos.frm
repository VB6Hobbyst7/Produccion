...
Dim lsUserOpeEspecial As String
'************************************************************************
...
Dim NumMenus As Integer
    ...
    If MsgBox("Esta Seguro que Desea Regenerar el Menu", vbInformation + vbYesNo, "Aviso") = vbYes Then
        ...
        Call oAcceso.RegeneraMenu(MatMenu, 0)
...
Private Sub cmdsalir_Click()
    Call oAcceso.CargaMenu(gsDominio, gsCodUser, , , , 0)
    Call CargaMenuMDIMain(oAcceso)
    Unload Me
End Sub

'**************** Add TORE: Adecuacion por HelpDesk ***************'
Public Sub InicioOpeEspecial(ByVal cUser)
    lsUserOpeEspecial = cUser
    Me.Show 1
End Sub
'******************************************************************'
...
'    Call CargaTVewOperaciones
    ...
    '**** add TORE: adecuacion por HelpDesk ****
    If (lsUserOpeEspecial <> "") Then
        Me.Caption = "Opciones Operaciones - " & UCase(lsUserOpeEspecial)
        CmdReGenerar.Visible = False
        SSFichas.Visible = False
        SSTabOpc.TabVisible(0) = False
        SSTabOpc.Top = 135
        SSTabOpc.Left = 105
        Me.Height = 4505
        Frame2.Top = 3355
...
'********************************************************************************
Call oAcceso.CargaControles(gsDominio, MatUser, MatGrupo, RsMenu, rsOpe, gsCodUser, rsOpcEsp, rsCargos, 0) 'AMDO 20130726, Se agrego RsOpcEsp,RsCargos
'*************END AMDO
...
'            Me.cmdagregar.Enabled = False
        End If
        Call oAcceso.CargaMenu(gsDominio, LstGrupos.SelectedItem.Text, "2", , , 0)
        sCadMenu = oAcceso.sCadMenu
...
'            Me.cmdagregar.Enabled = True
        ...
        'Call oAcceso.CargaMenu(gsDominio, LstUsuarios.SelectedItem.Text, "1", , gdFecSis, 0) 'Comento TORE 20200710: Adecuacion HelpDesk
        Call oAcceso.CargaMenu(gsDominio, IIf(lsUserOpeEspecial <> "", lsUserOpeEspecial, LstUsuarios.SelectedItem.Text), "1", , gdFecSis, 0) 'Mod TORE 20200710: Adecuacion HelpDesk
        'END MADM
        sCadMenu = oAcceso.sCadMenu
        sCadMenuGrp = oAcceso.sCadMenuGrp
                ...
                If Not Node.Checked Then
                    If IsDate(txtFecha) Then
                        If (txtFecha >= gdFecSis) Then
                            Call oAcceso.OtorgarOperacion(sUsuario, Node.Tag, sTipoUsu, CDate(Me.txtFecha.Text), 0)
                        Else
                            MsgBox "La fecha ingresada debe ser Mayor o Igual a la del Sistema", vbInformation, "Aviso"
                            Exit Sub
                        End If
                    Else
                        Call oAcceso.OtorgarOperacion(sUsuario, Node.Tag, sTipoUsu, , 0)
                    End If
...
Dim sUsuario As String
        ...
        If UCase(Node.Text) <> "MENU" Then
            If bChecked = True Then
                ...
                End If
            Else
                Call oAcceso.DenegarOperacion(sUsuario, Node.Tag, sTipoUsu, 0)
                Node.Checked = False
            End If
        End If
        If oAcceso.vError Then
            ...
            If Node.Checked = True Then
              If IsDate(txtFecha) Then
                    If (txtFecha >= gdFecSis) Then
                        ...
                        Call oAcceso.OtorgarAcceso(sUsuario, Node.Tag, sTipoUsu, CDate(Me.txtFecha.Text), 0) 'WIOR 20130913
                        '***Agregado por ELRO el 20111130, según Acta 324-2011/TI-D
                ...
                Else
                    ...
                    Call oAcceso.OtorgarAcceso(sUsuario, Node.Tag, sTipoUsu, , 0) 'WIOR 20130913
                    '***Agregado por ELRO el 20111130, según Acta 324-2011/TI-D
                    Set oNCOMContFunciones = New COMNContabilidad.NCOMContFunciones
                    lsMovNro = oNCOMContFunciones.GeneraMovNro(gdFecSis, gsCodAge, gsCodUser)
                    Call oPista.InsertarPista(gsOpeCod, lsMovNro, gsCodPersUser, GetMaquinaUsuario, "1", "Otorga Permiso a " & IIf(sTipoUsu = "1", "USUARIO " & UCase(sUsuario), UCase(sUsuario)) & " la opción " & CStr(Node.Tag) & " - " & Replace(Node.Text, "&", "") & " cuya ruta es " & Replace(CStr(Node.FullPath), "&", ""))
...
'Add TORE 20200710: Adecuacion a pedido de HelpDesk
Private Function ValidaFechaOpcionEspecial() As Boolean
    If (lsUserOpeEspecial <> "") Then
        If (txtFecha.Text = "__/__/____") Then
            MsgBox "Debe ingresar la fecha antes de proceder con la selección de las opciones", vbInformation, "Opcion no registrada"
            txtFecha.SetFocus
            ValidaFechaOpcionEspecial = False
            Exit Function
        ElseIf (Not IsDate(txtFecha.Text)) Then
            MsgBox "La fecha ingresada no tiene el formato correcto", vbInformation, "Opcion no registrada"
            txtFecha.SetFocus
            ValidaFechaOpcionEspecial = False
            Exit Function
        End If
    End If
    ValidaFechaOpcionEspecial = True
End Function
'**************************************************

Private Sub TVOperacion_NodeCheck(ByVal Node As MSComctlLib.Node)
'Add TORE 20200710: Adecuacion a pedido de HelpDesk
If Not ValidaFechaOpcionEspecial() Then
    Exit Sub
End If
'**************************************************
Dim sTipoUsu As String
...
'***Fin Agregado por ELRO**********************************
        ...
        If SSFichas.Tab = 0 Then
            sTipoUsu = "1"
            'sUsuario = Trim(LstUsuarios.SelectedItem.Text)
            sUsuario = IIf(lsUserOpeEspecial <> "", lsUserOpeEspecial, Trim(LstUsuarios.SelectedItem.Text)) 'Add TORE 20200710: Adecuacion por pedido de HelpDesk
        Else
            sTipoUsu = "2"
            sUsuario = Trim(LstGrupos.SelectedItem.Text)
        End If
