...
Begin VB.Form frmGiroMantDestinatario 
   ...
   Begin VB.CommandButton cmdClave 
      Caption         =   "Cambiar Clave Seg. PIN PAD"
      Enabled         =   0   'False
      Height          =   375
      Left            =   3480
      TabIndex        =   35
...
Private Sub CmdAceptar_Click()
    ...
    Dim nMonto As Double 'NAGL 20181121
    Dim sCuenta As String
    ...
    nMonto = Me.lblMonto.Caption 'NAGL 20181121 Según RFC1807260001
    ...
    If GClaveGiro <> Me.txtClaveAnt.Text Then
       MsgBox "Clave de seguridad no válida. Ingrese la clave correcta", vbInformation, "Aviso"
       Exit Sub
    End If
    ...
    clsServ.ServGiroCambioDestino Me.txtCuenta.NroCuenta, sMovNro, nMonto, nMontoComis, nTpoClienteAnt, nTpoClienteAct, sAgenciaDest, Right(Me.lblAgenciaDes.Caption, 2), Me.txtBuscaPersona.Text, txtDNI.Text, sPersCodAnt, sNumDOIAnt, Me.txtNombre.Text, txtReferencia.Text, Me.txtClaveNew.Text, lsBoleta, sRemitenteNomb, Me.cboAgencia.Text
    'NAGL 20181121 Cambió de Val(Me.lblMonto.Caption) a nMonto
    Set clsServ = Nothing
...
Dim lsclaveAnt As String
'Dim lsClaveNew As String 'COMENTADO POR JHCU
'Dim lsClaveNewConf As String 'add jhcu
Dim bClavNew As Boolean 'add jhcu
Dim bClavAnt As Boolean 'add jhcu
    
    txtClaveAnt.PasswordChar = "*"
    txtClaveNew.PasswordChar = "*"
     bClavNew = False 'add jhcu
    lsNumTarjGir = "4697100000000025"
    lsclaveAnt = ""
    lsclaveAnt = Tarjeta.PedirPinEnc(lsNumTarjGir, gNMKPOS, gWKPOS, 1, gnPinPadPuerto)
    
 
  
    If lsclaveAnt <> "" And Len(lsclaveAnt) = 16 Then
    
        bClavAnt = validarclaveAntigua(lsclaveAnt)
        
        If bClavAnt = True Then   'ADD JHCU
        txtClaveAnt.Text = lsclaveAnt
        MsgBox "Ingrese Nueva Clave de Seguridad", vbInformation, "SICMACM - Operaciones"
        ingresaNuevaClave
       
        Else 'ADD JHCU
         MsgBox "Clave de Seguridad incorrecta, por favor ingrese nuevamente la contraseña", vbInformation, "SICMACM - Operaciones"
         cmdClave_Click
        End If 'ADD JHCU
    Else
        'Me.lblClave.Caption = "NO INGRESADO"
          MsgBox lsclaveAnt & " con el PINPAD", vbInformation, "SICMACM - Operaciones"
    End If
   
    
    
End Sub
Private Function validarclaveAntigua(lsclaveAnt As String) As Boolean  'ADD JHCU
  validarclaveAntigua = False
  If lsclaveAnt <> "" And Len(lsclaveAnt) = 16 Then
        If GClaveGiro = lsclaveAnt Then
        txtClaveAnt.Text = lsclaveAnt
        validarclaveAntigua = True
        End If
  End If

End Function

Public Sub ingresaNuevaClave()
 Dim lsClaveNew As String
 Dim lsClaveNewConf As String 'add jhcu
 lsClaveNew = ""
 lsClaveNew = Tarjeta.PedirPinEnc(lsNumTarjGir, gNMKPOS, gWKPOS, 1, gnPinPadPuerto)

 
  If lsClaveNew <> "" And Len(lsClaveNew) = 16 Then
        MsgBox "Por favor ingrese nuevamente la contraseña para su confirmación", vbInformation, "SICMACM - Operaciones Nueva Contraseña"
      
    Else
        MsgBox lsClaveNew & " con el PINPAD", vbInformation, "SICMACM - Operaciones"
        Exit Sub
    End If
 
  lsClaveNewConf = Tarjeta.PedirPinEnc(lsNumTarjGir, gNMKPOS, gWKPOS, 1, gnPinPadPuerto)
     
 If lsClaveNewConf <> "" And Len(lsClaveNewConf) = 16 Then
        If lsClaveNewConf = lsClaveNew Then
              MsgBox "Nueva Clave de seguridad", vbInformation, "SICMACM - Operaciones"
              txtClaveNew.Text = lsClaveNew
        Else
              MsgBox "Las contraseñas no coinciden, por favor ingrese la contraseña nuevamente", vbInformation, "SICMACM - Operaciones Nueva Contraseña"
              ingresaNuevaClave
        End If
 Else
    MsgBox lsClaveNewConf & " con el PINPAD", vbInformation, "SICMACM - Operaciones"
 End If
End Sub
