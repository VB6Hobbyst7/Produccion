...
On Error GoTo error_handler
    ...
    dlgArchivo.Filter = "Archivos BCP (*.TXT)|*.TXT"
    dlgArchivo.ShowOpen
    If dlgArchivo.FileName <> Empty Then
        ...
        Screen.MousePointer = 0
    Else
        MsgBox "No se seleccionó ningún archivo", vbExclamation, "Aviso"
        Exit Sub
    End If
...
Public Sub Leer_Lineas(strTextFile As String)
    ...
    Dim oTmp As COMDCredito.DCOMCreditos 'CTI2 20181205
    Dim ultm As Integer 'CTI2 20181205
    ...
    If Not rsx.EOF And Not rsx.BOF Then
        cod_det = rsx("id")
        MsgBox "Se van a procesar " & (RegActual2 - rsx!cantRegistros) & " registros, Esta operación puede demorar unos minutos ...", vbInformation, "Aviso"
    Else
        MsgBox "El archivo no tiene la estructura correcta", vbInformation, "Aviso"
        Exit Sub
    End If
    Set oTmp = New COMDCredito.DCOMCreditos
    For iMat = 1 To UBound(MatDet, 2)
        ...
        With DatosD
            .d1 = MatDet(1, iMat) 'Tregistro
            .d2 = MatDet(2, iMat) 'CSucursal
            .d3 = MatDet(3, iMat) 'TMoneda
            .d4 = MatDet(4, iMat) 'NCuenta
            .d5 = MatDet(5, iMat) 'NCodCuenta
            .d6 = MatDet(6, iMat) 'CRetorno
            .d7 = MatDet(7, iMat) 'FPago
            .d8 = MatDet(8, iMat) 'FProceso
            .d9 = MatDet(9, iMat) 'Total
            .d10 = MatDet(10, iMat) 'MontoMora
            .d11 = MatDet(11, iMat) 'MontoTotal
            .d12 = MatDet(12, iMat) 'COficina
            .d13 = MatDet(13, iMat) 'NOperacion
            .d14 = MatDet(14, iMat) 'NReferencia
            .d15 = MatDet(15, iMat) 'Iterminal
            .d16 = MatDet(16, iMat) 'MAtencion
            .d17 = MatDet(17, iMat) 'HAtencion
            .d18 = MatDet(18, iMat) 'NNumeroCh
            .d19 = MatDet(19, iMat) 'CCodBco
            .d20 = MatDet(20, iMat) 'CCargoF
            .d21 = MatDet(21, iMat) 'ValorE
            
            'CTI2 *********
            nImporteCuota = Format(Mid(DatosD.d9, 1, Len(DatosD.d9) - 2), ".") + Right(DatosD.d9, 2)
            If Not oTmp.VerificaItemTramaBCP(DatosC.c6, .d5, nImporteCuota, .d13, .d12) Then
                Call LeerDetalle
            Else
...
Public Function LeerCabecera() As Boolean
    ...
    End With
       ...
       If bValorProFecha Then
            ...
            Else
            'CTI2 20181204 *********
              If rsx!CRegistros = rsx!cantRegistros Then
...
Sub LeerDetalle()
    ...
    End With
       ...
       If Not rsx.EOF And Not rsx.BOF Then
            ...
            'Call oCredD.InsertarDatosDetalleBCP(cod_det, rsBCPCD)  Comentado por CTI2
            Call ConnBase.InsertarDatosDetalleBCP(cod_det, rsBCPCD) 'CTI2
            Set oCredD = Nothing
            Set rsy = ConnBase.getDatosDetalleBcoBCPxId(cod_det, rsBCPCD.Fields("cCodCtaF"), rsx("nCuota"))
            ...
            If Not CargaDatos(cod_dety, rsBCPCD.Fields("cCodCtaF"), CDbl(rsBCPCD.Fields("c11")), CStr(Format(gdFecSis, "YYYYMMDD")), rsBCPCD.Fields("Cuota"), ConnBase) Then
                   Dim ultm As Integer
                   ...
                   'Call oGen.ActualizarDetallexEstBCP(cod_det, rsBCPCD.Fields("cCodCtaF"), rsBCPCD.Fields("Cuota"), gdFecSis, 1)
                    ConnBase.dRollbackTrans 'ADD CTI2 20181205
                     rsBCPCD.Close
                     Set rsBCPCD = Nothing
...
Private Function CargaDatos(ByVal cod_det As Double, ByVal psCtaCod As String, _
                            ByVal pnImporteCobrado As Double, ByVal pdFechaCobro As String, _
                            ByVal pnNroCta As Integer, _
...
Dim ultm As Integer
    ...
    nDiasAtras = DateDiff("d", _
                          CDate(Mid(DatosD.d7, 1, 4) & "-" & Mid(DatosD.d7, 5, 2) & "-" & Mid(DatosD.d7, 7, 2)), _
                          gdFecSis) 'RIRO 20210526 ADD ICV
    ...
    If Not rsCredVig.BOF And Not rsCredVig.EOF Then
        ...
        If bValorProceso Then
               Dim sError As String ' CTI2 20181205
               ...
               sCadImpre = sCadImpre & sImpreBoleta
                ...
                pTrans.OtrasOpeDepCtaBanco sMovNro, "01.1090100824640.0110006", psCtaCod, MontoPagosinITF + nITF, cNomCliente, gsNomAge, IIf(LblMoneda = "SOLES", 1, 2), 300593
                ...
                If LblMoneda = "SOLES" Then
                   ...
                   'oCredD.GrabaDeposiBancoNegocioFinan lsMovNro, "401583", "Deposito Banco", "1113010301",
                    'oCredD.GrabaDeposiBancoNegocioFinan lsMovNro, "401583", psMovDesc, "1113010301",
                    pTrans.GrabaDeposiBancoNegocioFinan lsMovNro, "401583", psMovDesc, "1113010301", _
                ...
                Else
                    ...
                    'oCredD.GrabaDeposiBancoNegocioFinan lsMovNro, "402583", psMovDesc, "1123010301",
                    pTrans.GrabaDeposiBancoNegocioFinan lsMovNro, "402583", psMovDesc, "1123010301", _
                    cod_ageContable, CCur(MontoPagosinITF + nITF), 0, "", pdFechaCobro, "", _
              ...
              'guardar Movs (pago, reversion y contable) --------------------------------------
                If NroMov > 0 And nMovNroR > 0 And nMovNroC > 0 Then
                   'Call oCredD1.ActualizarDetallexEstMovBCP(cod_det, NroMov, nMovNroR, nMovNroC) CTI2 comentado 20181015
                   Call pTrans.ActualizarDetallexEstMovBCP(cod_det, NroMov, nMovNroR, nMovNroC)
                        If (dif - nITF) > 0 Then
                            'Call oCredD1.InsertarDatosBCPSobrantes(psCtaCod, nNroCta, dif - nITF, FechaCobro1, cNomCliente, NroMov) CTI2 20181015 comentado
                            Call pTrans.InsertarDatosBCPSobrantes(psCtaCod, nNroCta, dif - nITF, FechaCobro1, cNomCliente, NroMov)
                        End If
                Else
                    CargaDatos = False 'CTI2 20181205
                End If
              '----------------------------------------------------------------------------------
               procesados = procesados + 1
               ultm = n_procesados + procesados
