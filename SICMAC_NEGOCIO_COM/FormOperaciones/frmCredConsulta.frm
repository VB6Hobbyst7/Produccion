...
Begin VB.Form frmCredConsulta 
   ...
   Begin TabDlg.SSTab SSTab1 
      ...
      Tab(0).Control(1)=   "CmdMuestra"
      Tab(0).Control(1).Enabled=   0   'False
      Tab(0).Control(2)=   "CmdHistorial"
      Tab(0).Control(2).Enabled=   0   'False
      Tab(0).Control(3)=   "cmdIdentificacionRCC"
...
'Segmentacion de riesgo JOEP20201118
    ...
    Do While Not rsPag.EOF
        ...
        nCapPag = nCapPag + CDbl(L.SubItems(3))
        L.SubItems(4) = Format(rsPag!nInteres, "#0.00")
        nIntPag = nIntPag + CDbl(L.SubItems(4))
        L.SubItems(5) = Format(rsPag!nMora, "#0.00")
        nMora = nMora + CDbl(L.SubItems(5))
        'L.SubItems(6) = Format(rsPag!nGastos, "#0.00")
        L.SubItems(6) = Format(rsPag!nGastoComision + rsPag!nGastoSegDes + rsPag!nGastoPolizaIncendio + rsPag!nGastoPolizaVehiculo, "#0.00") 'EJVG20140930
        nGasto = nGasto + CDbl(L.SubItems(6))
        'EJVG20140930 ***
        L.SubItems(7) = Format(rsPag!nGastoComision, "#0.00")
        L.SubItems(8) = Format(rsPag!nGastoSegDes, "#0.00")
        L.SubItems(9) = Format(rsPag!nGastoPolizaIncendio, "#0.00")
        L.SubItems(10) = Format(rsPag!nGastoPolizaVehiculo, "#0.00")
        'END EJVG *******
        L.SubItems(11) = Format(rsPag!nITF, "#0.00")
        nITF = nITF + CDbl(L.SubItems(11))
            
        L.SubItems(12) = Format(rsPag!nDiasMora, "#0")
        L.SubItems(13) = Format(rsPag!nSaldoCap, "#0.00")
        L.SubItems(14) = IIf(IsNull(rsPag!cUsuario), "", rsPag!cUsuario)
        rsPag.MoveNext
    Loop
    ...
    For i = 0 To UBound(MatCalend) - 1
        ...
        If RDatos!nPrdEstado = gColocEstCancelado Or RDatos!nPrdEstado = gColocEstRefinanc Then
            L.SubItems(12) = DateDiff("d", CDate(MatCalend(i, 0)), RDatos!dPrdEstado)
        ...
        'ALPA 20110329***************************
            If (IIf(IsNull(RDatos!nDiasAtraso), 0, RDatos!nDiasAtraso) = 0 And i = 0) And DateDiff("d", RDatos!dVigencia, gdFecSis) <> 0 Then
                L.SubItems(12) = 0
            Else
                L.SubItems(12) = DateDiff("d", CDate(MatCalend(i, 0)), gdFecSis)
            End If
        End If
        '*****************************************************************************
        L.SubItems(5) = MatCalend(i, 9)
        'EJVG20140925 ***
        L.SubItems(6) = MatCalend(i, 13) 'Gasto Comisión(Todos los gastos no especificados)
        L.SubItems(7) = MatCalend(i, 14) 'Seg. Desgravamen
        L.SubItems(8) = MatCalend(i, 15) 'Seg. contra Incendio
        L.SubItems(9) = MatCalend(i, 16) 'Seg. Vehicular Multiriesgo
        L.SubItems(10) = MatImpuesto(i)
        L.SubItems(11) = Format(CDbl(MatCalend(i, 3)) + CDbl(MatCalend(i, 4)) + CDbl(MatCalend(i, 5)) + CDbl(MatCalend(i, 6)) + CDbl(MatCalend(i, 7)) + CDbl(MatCalend(i, 11)) + CDbl(MatCalend(i, 8)) + CDbl(MatCalend(i, 9)) + CDbl(MatImpuesto(i)), "#0.00")
        L.SubItems(13) = MatCalend(i, 10)
    ...
    lblTotalFecha.Caption = sTotalFecha
   ...
   'Carga Cuotas Pagadas
    ...
    If Not rsPagos Is Nothing Then
        If Not rsPagos.EOF And Not rsPagos.BOF Then
            ReDim MatPagos(rsPagos.RecordCount, 11)
    ...
    ReDim MatCuotasPend(lstCuotasPend.ListItems.count, 8)
    For i = 0 To lstCuotasPend.ListItems.count - 1
        ...
        MatCuotasPend(i, 5) = lstCuotasPend.ListItems(i + 1).SubItems(12) 'EJVG20140925
        'MatCuotasPend(i, 6) = lstCuotasPend.ListItems(i + 1).SubItems(6)
        MatCuotasPend(i, 6) = lstCuotasPend.ListItems(i + 1).SubItems(10) 'EJVG20140925
        'MatCuotasPend(i, 7) = lstCuotasPend.ListItems(i + 1).SubItems(9)
        MatCuotasPend(i, 7) = lstCuotasPend.ListItems(i + 1).SubItems(13) 'EJVG20140925
        'MatCuotasPend(i, 8) = lstCuotasPend.ListItems(i + 1).SubItems(7)
        MatCuotasPend(i, 8) = lstCuotasPend.ListItems(i + 1).SubItems(11) 'EJVG20140925
    Next i
