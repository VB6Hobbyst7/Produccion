...
Begin VB.Form frmSegSepelioAfiliacion 
   ...
   Begin VB.Frame frSegSepelio1 
      ...
      Top             =   2160
      Width           =   8295
      Begin VB.TextBox txtUsuaSepelio 
         Alignment       =   2  'Center
         Enabled         =   0   'False
         BeginProperty Font 
            Name            =   "MS Sans Serif"
         ...
         Width           =   975
      End
      Begin VB.CheckBox chkActivarSepelio 
         Height          =   255
         Left            =   960
         TabIndex        =   34
         Top             =   1020
   ...
   Begin ComctlLib.TabStrip TabStrip1 
      ...
      EndProperty
   End
   Begin SICMACT.Usuario UserSeguros 
      Left            =   840
      Top             =   7680
      _ExtentX        =   820
      _ExtentY        =   820
...
Dim cCodOpeEncuesta As String 'add jhcu 2810-2019
...
Private Sub CmdAceptar_Click()
    ...
    'CTI2 FERIMORO : ERS034-2019
        If (chkActivarSepelio.value = 1 And (lblUsuaSepelio.Caption = "" Or txtUsuaSepelio = "")) Or Len(txtUsuaSepelio) < 4 Then
           MsgBox "Debe agregar el Usuario del Promotor del Seguro", vbInformation, "Alerta SICMAC"
           CmdAceptar.Enabled = True
           txtUsuaSepelio.SetFocus
           Exit Sub
        End If
    '***********************************
    ...
    If bTarj = True Or bSepl = True Then
        ...
        Encuestas gsCodUser, gsCodAge, "ERS0292019", cCodOpeEncuesta
        'Fin
        Unload Me
    End If
...
'        End If
        ...
        If oNCapMov.ValidaSaldoCuenta(sCuenta, nMonto, gAhoCargoAfilSegSepelio) Then
            ...
            If MsgBox("Se va a realizar el Cargo a la cuenta por Afiliación de Seguro de Sepelio, Desea Continuar?", vbInformation + vbYesNo, "Aviso") = vbNo Then Exit Function
                'ADD BY PTI1 20210926
                Set oNSegTar = New COMNCaptaGenerales.NCOMSeguros
                If Not oNSegTar.ValidaPerSegSepelioA(sCuenta, fsPersCod) Then
                    MsgBox "El N° de Cuenta: " & sCuenta & " no pertence al cliente " & fsPersNombre, vbCritical, "Aviso"
                    Call LimpiarFormulario
                    Exit Function
                End If
                ...
                nSaldo = oNCapMov.CapCargoCuentaSegSepelio(sCuenta, nMonto, sCodOpe, sMovNro, "Afiliacion Seguro Sepelio Cuenta = " & sCuenta, gsNomAge, sLpt, gsCodCMAC, gsCodAge, lsmensaje, gbImpTMU, Trim(txtCertificadoSepelio.Text), sNumTarj, gdFecSis, feAsegurado.TextMatrix(1, 1), lsBoleta, gsCodAge, nMovNro, , ChkPrimaAnualSS.value, gsCodUser, txtUsuaSepelio)
            '    cUser As String = "", Optional ByVal cUserVenta As String = ""
                ...
                ''CTI2 FERIMORO ERS034-2019 25082020 : COMENTADO
                Dim clsCapMov As New COMNCaptaGenerales.NCOMCaptaMovimiento
                Dim nProxFechaB As String
                nProxFechaB = clsCapMov.cnsSeguroCronograma(Trim(txtCertificadoSepelio.Text))
                Dim rsPagos As ADODB.Recordset
                Set rsPagos = New ADODB.Recordset
                
                With rsPagos.Fields
                    .Append "mes", adBSTR, 10
                    .Append "monto", adBSTR, 30
                    .Append "nPrima", adBSTR, 10
                End With
                rsPagos.Open
                
                 rsPagos.AddNew _
                    Array("mes", "monto", "nPrima"), _
                    Array("", "S/. " & CStr(Format(nMonto, "#,##0.00")), nMonto)
                rsPagos.MoveFirst
                ...
                lsBoleta = oNSegSep.ImprimeBoletaAfilicacionSeguroSepelio(nMovNro, sMovNro, gsNomAge, gbImpTMU, , nProxFechaB, rsPagos) 'CTI2 FERIMORO ERS034-2019 25082020 : COMENTADO
                Do
                    nFicSal = FreeFile
                    Open sLpt For Output As nFicSal
                    Print #nFicSal, lsBoleta & oImpresora.gPrnSaltoLinea & oImpresora.gPrnSaltoLinea & oImpresora.gPrnSaltoLinea
...
Private Function RealizarPagoSinDebitoCta() As Boolean
    ...
    On Error GoTo Error
        ...
        If MsgBox("¿Desea Grabar la Información?", vbQuestion + vbYesNo, "Aviso") = vbYes Then
            ...
            'clsCapM.AgregaSegSepelioAfiliacion txtCertificadoSepelio.Text, "", gdFecSis, lsMov, lnMovNro, feAsegurado.TextMatrix(1, 1), gsCodAge, ChkPrimaAnualSS.value  'COMENTADO CTI2 FERIMORO : 15012021
            clsCapM.AgregaSegSepelioAfiliacion txtCertificadoSepelio.Text, "", gdFecSis, lsMov, lnMovNro, feAsegurado.TextMatrix(1, 1), gsCodAge, ChkPrimaAnualSS.value, gsCodUser, txtUsuaSepelio 'ADD: CTI2 FERIMORO : 15012021
            'APRI20181023 ERS071-2018 ADD ChkPrimaAnualSS.value
...
'*************************************************
Sub carga_usuarios()
UserSeguros.Inicio gsCodUser
 
txtUsuaSepelio.Text = gsCodUser
lblUsuaSepelio.Caption = UserSeguros.UserNom
End Sub

Private Sub chkActivarSepelio_Click()
If chkActivarSepelio.value = 1 Then
 txtUsuaSepelio.Enabled = True
 txtUsuaSepelio.Text = ""
 lblUsuaSepelio.Caption = ""
 txtUsuaSepelio.SetFocus
Else
    txtUsuaSepelio.Enabled = False
    txtUsuaSepelio.Text = gsCodUser
    UserSeguros.Inicio txtUsuaSepelio
    lblUsuaSepelio = UserSeguros.UserNom
End If

End Sub

Private Sub txtCertificadoSepelio_KeyPress(KeyAscii As Integer)

    KeyAscii = SoloNumeros(KeyAscii)

End Sub

Private Sub txtUsuaSepelio_Change()
    txtUsuaSepelio = UCase(txtUsuaSepelio)
    txtUsuaSepelio.SelStart = Len(txtUsuaSepelio.Text)
    If txtUsuaSepelio <> "" Then
        UserSeguros.Inicio txtUsuaSepelio
        lblUsuaSepelio = UserSeguros.UserNom
    End If
End Sub

'Private Sub txtUsuaTarjeta_Change()
'txtUsuaTarjeta = UCase(txtUsuaTarjeta)
'txtUsuaTarjeta.SelStart = Len(txtUsuaTarjeta.Text)
'If txtUsuaTarjeta <> "" Then
'    UserSeguros.Inicio txtUsuaTarjeta
'    lblUsuTarjeta = UserSeguros.UserNom
'End If
'End Sub
'************************************************
