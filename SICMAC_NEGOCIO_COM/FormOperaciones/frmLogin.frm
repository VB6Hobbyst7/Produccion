...
'Dim oSeguridad As New COMManejador.Pista
    ...
    Call oAcceso.CargarLogin_NEW(Trim(LblUsu.Caption), Trim(TxtClave.Text), bInterconexCorrecta, bClaveIncorrecta, rsITF, rsVar, psServerName, psDBName, psCadConexion, _
                              pMatMenuPer, bTieneAlgunPermiso, bTienePermisoRetiroSinFirma, bIniciarNuevoDia, gRsOpeF2, gRsExtornos, gRsOpeCMACRecep, gRsOpeCMACLlam, gRsOpeRepo, 0)
    

    'Set oAcceso = Nothing
    
    'AGREGADO POR ARLO20170331
    Call CargaVarSistema(False, rsVar, psServerName, psDBName, psCadConexion)
    gsSistDescripcion = "SICMACM Operaciones"
    gnSistCod = 0
    gsFechaVersion = "20211020"                 'CAMBIAR LA FECHA CADA VEZ QUE SE COMPILA
    'AGREGADO POR ARLO20170331
    ...
    If Not bClaveIncorrecta Then
        ...
        'AGREGADO POR ARLO20170331
            ...
            Call oSeguridad.InsertarPista(gIngresarSalirSistema, GeneraMovNroPistas(gdFecSis, gsCodAge, gsCodUser), gsCodPersUser, GetMaquinaUsuario, gIngresarSistema, "Intento Fallido de Ingreso al " & gsSistDescripcion & " Versión: " & Format(App.Major, "#0") & "." & Format(App.Minor, "#0") & "." & Format(App.Revision, "#0") & "-" & gsFechaVersion)
            If UsRf Then
    ...
    'Call CargaVarSistema(False, rsVar, psServerName, psDBName, psCadConexion) COMENTADO POR ARLO20170331
    ...
    SetPermisosAtajosTeclado gRsOpeF2.Clone 'APRI20161117
    ...
    MDISicmact.Caption = "Operaciones - " & Trim(gsNomAge) & " - " & Trim(gsCodUser) & space(10) & gsServerName & "\" & gsDBName & space(5) & Format(gdFecSis, "dd/mm/yyyy")
...
''    TxtClave.SetFocus
    ...
    'Carga DMONT para Pin Pads de marca UNISYS
    
    'ARCV 22-02-2007
    'Dim nRetVal As Long, X As Double
    'ChDrive App.path
    'ChDir App.path
...
Dim sMaquinaActiva As String 'JUEZ 20160405
    ...
    If oSeguridad.ValidaAccesoPistaRF(gsCodUser) Then
        ...
        acceso = oSeguridad.ValidaIniciaSessionRF(gsCodPersUser, gdFecSis, GetMaquinaUsuario, 0, sMaquinaActiva) 'JUEZ 20160125
        If acceso Then
            MsgBox "No puede Acceder al Sistema porque tiene una Sesion Abierta en otra PC. Consulte con Help Desk", vbExclamation, "Ingreso SICMACM Operaciones"
            ...
            EnviarMail gsCorreoHost, gsCorreoEnvia, lsCorreoDestino, "Validación Acceso SICMACM Operaciones", lsContenido
            'Fin Envio Correo ****************************************************
...
'END JUEZ *********************************************************************
'EJVG20161116 ***
Private Sub SetPermisosAtajosTeclado(ByVal pRsF2 As ADODB.Recordset)
    If pRsF2.RecordCount > 0 Then
        Do While Not pRsF2.EOF
            Select Case pRsF2!cOpecod
                Case "121100"
                    gAtajoTeclado.bRenovaPigno = True
                Case "210201"
                    gAtajoTeclado.bRetiroIntDPF = True
                Case "901013"
                    gAtajoTeclado.bHabEfectivo = True
                Case "301000"
                    gAtajoTeclado.bDebitoSP = True
                Case "901016"
                    gAtajoTeclado.bCuadre = True
                Case "210301"
                    gAtajoTeclado.bCancelaDPF = True
                Case "121200"
                    gAtajoTeclado.bCancelaPigno = True
                Case "900022"
                    gAtajoTeclado.bCompraME = True
                Case "200301"
                    gAtajoTeclado.bRetiroEfectAho = True
                Case "100101"
                    gAtajoTeclado.bDesembEfect = True
                Case "901017"
                 gAtajoTeclado.bConfirHab = True
                Case "100102"
                    gAtajoTeclado.bDesembCta = True
                Case "200201"
                     gAtajoTeclado.bDepoEfectAHO = True
                Case "900023"
                    gAtajoTeclado.bVentaME = True
                Case "310101"
                    gAtajoTeclado.bAperGiro = True
                Case "220301"
                    gAtajoTeclado.bRetiroCTS = True
                Case "300120"
                    gAtajoTeclado.bPagoSERV = True
                Case "100201"
                    gAtajoTeclado.bPagoCRED = True
                Case "210801"
                    gAtajoTeclado.bAumentoCapDPF = True
                Case "310301"
                    gAtajoTeclado.bCancelGiro = True
            End Select
