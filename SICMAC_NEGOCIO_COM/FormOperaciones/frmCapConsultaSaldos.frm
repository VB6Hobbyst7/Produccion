...
Dim fsBoleta As String
Dim nMontoCom As Currency
...
If Not (rsCta.EOF And rsCta.BOF) Then
    ...
    nMontoCom = 0
    If Not CargoAutomatico(sCuenta, 1) Then Exit Sub 'JUEZ 20151229
    'END JUEZ ***************************************
    ...
    Else
        ...
        nSaldoDisp = rsCta("nSaldoDisp") - nMontoCom  'JUEZ 20160326
        'nSaldoCnt = rsCta("nSaldo")
        nSaldoCnt = rsCta("nSaldo") - nMontoCom 'JUEZ 20160326
    ...
    If nmoneda = gMonedaNacional Then
        ...
        For i = 0 To 3
            'lblMon(i) = "S/."
            lblMon(i) = gcPEN_SIMBOLO 'APRI20191022 SUGERENCIA CALIDAD
        Next i
    Else
    ...
    frmSegSepelioAfiliacion.Inicio sCuenta 'JUEZ 20151229
Else
...
If KeyAscii = 13 Then
    ...
    'End If
    ObtieneDatosCuenta sCta
    'frmSegSepelioAfiliacion.inicio sCta 'Comentado por JUEZ 20151229
    'END JUEZ ********************************************************************
End If
End Sub
    ...
    If bAplicar Then
        ...
        If nPersoneria <> gPersonaJurCFLCMAC Then 'Que no sean CMACs
            If nEstado <> gCapEstAnulada Then
                ...
                If nmoneda = gMonedaNacional Then
                    'nMontoCom = GetMontoDescuento(2106, pnCntPag)
                    nMontoCom = GetMontoDescuento(2106, pnCntPag, psCuenta) 'APRI20190109 ERS077-2018
                Else
                    'JUEZ 20160326 *************************************
                    'nMontoCom = GetMontoDescuento(2112, pnCntPag)
                    ...
                    'Set rs = ObjTc.GetTipoCambioCV(0)
                    'If Not (rs.EOF And rs.BOF) Then
                    '    nMontoCom = Round(GetMontoDescuento(2106, pnCntPag) / rs!nVenta, 2)
                    'End If
                    'Set rs = Nothing
                    'Set ObjTc = Nothing
                    'END JUEZ ******************************************
                    nMontoCom = Round(GetMontoDescuento(2112, pnCntPag, psCuenta), 2) 'APRI20190109 ERS077-2018
                End If
                If nMontoCom > 0 Then
                    ...
                    nITF = fgITFCalculaImpuesto(nMontoCom)
                    ...
                    If Mid(psCuentaImp, 6, 3) = gCapPlazoFijo Then
                        bValidaSaldo = False
                    Else
                        bValidaSaldo = oCap.ValidaSaldoCuenta(psCuentaImp, nMontoCom + nITF)
                    End If
                    
                    If bValidaSaldo Then
                         If MsgBox("Se va a realizar el cargo automático por la consulta de Saldos, Desea Continuar?", vbYesNo, "Aviso") = vbNo Then Exit Function
                    Else
                        MsgBox "Esta opción generará un cobro de comisión de " & Format(nMontoCom, "#,##0.00") & ", por favor elija la forma de pago", vbInformation, "Aviso"
                        psCuenta = frmOpeComisionConsultaMovs.Inicia(1, nMontoCom, nmoneda, rsClientes, nTipoPago, sPersCod, sPersNombre)
                    ...
                    'JUEZ 20151229 ****************************************************************************************************
                    'nFlag = oCap.CapCargoCuentaAho(psCuenta, nMontoCom, gAhoConsSaldo, sMovNro, "Descuento Emisión Extracto", , , , , , , , , gsNomAge, sLpt, , , , , gsCodCMAC, , gsCodAge, , , nitf, , , , , sMensaje, sBoleta, sBoletaITF, , , gbImpTMU)
                    If nTipoPago = 1 Then
                        nFlag = oCap.OtrasOperaciones(sMovNro, "300441", nMontoCom, "", "Comisión Diversa : Descuento Emisión Consulta Cuenta " & psCuentaImp, nmoneda, sPersCod)  'JUEZ 20151019
                        Dim oBol As COMNCaptaGenerales.NCOMCaptaImpresion
                        Set oBol = New COMNCaptaGenerales.NCOMCaptaImpresion
                            fsBoleta = oBol.ImprimeBoleta("COMISIONES DIVERSAS", Left("Descuento Consulta Saldos", 36), "", Str(nMontoCom), sPersNombre, "________1", "", 0, "0", "", 0, 0, False, False, , , , False, , , , gdFecSis, gsNomAge, gsCodUser, sLpt, , False, 0)
                        Set oBol = Nothing
                    Else
                        If Mid(psCuenta, 6, 3) = gCapAhorros Then
                            nFlag = oCap.CapCargoCuentaAho(psCuenta, nMontoCom, gAhoConsSaldo, sMovNro, "Descuento Emisión Consulta Cuenta " & psCuentaImp, , , , , , , , , gsNomAge, sLpt, , , , , gsCodCMAC, , gsCodAge, , False, nITF, , , , , sMensaje, fsBoleta, sBoletaITF, , , gbImpTMU)
                        ElseIf Mid(psCuenta, 6, 3) = gCapCTS Then
                            nFlag = oCap.CapCargoCuentaCTS(psCuenta, nMontoCom, gCTSConsSaldo, sMovNro, "Descuento Emisión Consulta Cuenta " & psCuentaImp, , , , , gsCodCMAC, , gsNomAge, sLpt, , , , , Mid(psCuenta, 9, 1), , , , , , sMensaje, fsBoleta, gbImpTMU)
                        End If
                    End If
                    'END JUEZ *********************************************************************************************************
                    Set oCap = Nothing
...
Private Function GetMontoDescuento(pnTipoDescuento As CaptacParametro, Optional pnCntPag As Integer = 0, Optional pcCtaCod As String = "") As Double
...
Set oParam = New COMNCaptaGenerales.NCOMCaptaDefinicion
    ...
    Set rsPar = oParam.GetTarifaParametro(nOperacion, nmoneda, pnTipoDescuento, pcCtaCod) 'APRI20190109 ERS077-2018
Set oParam = Nothing
