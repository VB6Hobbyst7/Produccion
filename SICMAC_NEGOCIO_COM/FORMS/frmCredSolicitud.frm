...
Begin VB.Form frmCredSolicitud 
   ...
   Begin VB.Frame fraCreditos 
      ...
      Begin VB.Frame fraLineaCred 
         ...
         Top             =   120
         Width           =   8970
         Begin VB.ComboBox cmbTpIngreso 
            Height          =   315
            Left            =   120
            Sorted          =   -1  'True
            Style           =   2  'Dropdown List
...
Dim nTpCmbTpIng As Integer 'ERS0032020
...
'    End If
    ...
    If cmbMoneda.ListIndex = -1 Then
        ...
        'Comento Joep20210304 Mejora cuando es un credito Pre-Solicitud
        If cmbMoneda.Enabled = True Then
            cmbMoneda.SetFocus
        End If
        'Comento Joep20210304 Mejora cuando es un credito Pre-Solicitud
        'cmbMoneda.SetFocus'Comento Joep20210304 Mejora cuando es un credito Pre-Solicitud
        ValidaDatos = False
        Exit Function
    End If
...
'If Trim(Right(cmbDestCred.Text, 5)) <> 14 Then 'Comento JOEP2090307 Mejora
    ...
    nTpIngr = IIf(cmbTpIngreso.Visible = True And cmbTpIngreso.Text <> "" And nTpCmbTpIng = 1, Trim(Right(cmbTpIngreso, 10)), 0)
...
End If
'CTI3 ERS0032020**************************************************
If Not CP_Mensajes(8, Trim(Right(cmbSubProducto.Text, 5))) Then
    cmbTpIngreso.SetFocus
    ValidaDatos = False
    Exit Function
End If
'*****************************************************************
...
'        End If
        ...
        'CTI3 ERS0032020
        CP_CargaTpTipoIngreso IIf(IsNull(R!cTpoProdCod), "0", R!cTpoProdCod), 0
        cmbTpIngreso.ListIndex = IndiceListaCombo(cmbTpIngreso, IIf(R!nTpIngr = -1, 0, R!nTpIngr))
...
Private Sub cmbTpIngreso_Click()
    If PersoneriaTitular <> gPersonaNat And CStr(Trim(Right(cmbTpIngreso.Text, 7))) = "34002" Then
        MsgBox "El cliente con personería Juridica no debe tener tipo de Ingreso Dependiente.", vbInformation, "Aviso"
        cmbTpIngreso.ListIndex = IndiceListaCombo(cmbTpIngreso, "34001")
    End If
End Sub
...
'
        ...
        End Select
    CP_CargaTpDoc Trim(Right(cmbSubProducto.Text, 10)), 48000
    CP_CargaTpTipoIngreso Trim(Right(cmbSubProducto.Text, 10)), 0
    Else
        CP_CargaTpTipoIngreso Trim(Right(cmbSubProducto.Text, 10)), 0
    End If
'Agrego JOEP20190919 ERS042 CP-2018
...
'Agrego JOEP20190919 ERS042 CP-2018
        ...
        If Right(Trim(cmbSubProducto.Text), 3) <> "703" Then
            cmbTpIngreso.Visible = True
            lblTipoIngreso.Visible = True
        Else
            cmbTpIngreso.Visible = False
...
Private Sub cmdEditar_Click()
    ...
    'CTI3 ERS0032020
    If cmbTpIngreso.Text = "" Then
      cmbTpIngreso.Enabled = True
    Else
      cmbTpIngreso.Enabled = False
    End If
    'END
    'ALPA 20100604 B2************************************
'Comento JOEP20190919 ERS042 CP-2018
...
'CTI3-ERS003-2020*****************************************
If Right(Trim(cmbSubProducto.Text), 3) <> "703" Then
    If Trim(cmbTpIngreso.Text) = "" Then
        MsgBox "Seleccione el Tipo de Ingreso.", vbInformation, "Aviso"
        If cmbTpIngreso.Enabled = True Then
            EnfocaControl cmbTpIngreso
            Exit Sub
        End If
    End If
End If
...
'**ARLO20180712 ERS042 - 2018
    ...
    If cmdEjecutar = 2 Then
        GenerarDataExposicionEsteCredito ActXCtaCred.NroCuenta, CDbl(txtMontoSol.Text), fnMontoExpEsteCred_NEW 'Seteamos el valor de la nueva exposición
           'CTI3 ERS0032020***********************************************************************
            ...
            Set rsFE = oEval.RecuperaCreditoxEvaluacion(ActXCtaCred.NroCuenta)
        ...
        If NecesitaFormatoEvaluacion(ActXCtaCred.NroCuenta, 2000, CInt(Right(cmbProductoCMACM.Text, 3)), CInt(Right(cmbSubProducto, 3)), fnMontoExpEsteCred_NEW, fbEliminarEvaluacion, nFormatoEliminado, nFormato_NEW, lsTpoCredCodPadre, nTpIngr) Then
        '******************************************************************************************
    ...
    'RIRO 20160509 Valida que los créditos ampliados/refinanciados tengan datos de liquidación *****
    Dim oCreditoTmp As COMNCredito.NCOMCredito
    Set oCreditoTmp = New COMNCredito.NCOMCredito
    Dim bResultLiquidacion As Boolean
    bResultLiquidacion = False
    
    'CREDITOS AMPLIADOS
    If bAmpliacion Then
        If Not rsAmpliado Is Nothing Then
            If rsAmpliado.State = 1 Then
                If Not rsAmpliado.EOF And Not rsAmpliado.BOF Then
                    If rsAmpliado.RecordCount > 0 Then
                        rsAmpliado.MoveFirst
                        Do While Not rsAmpliado.EOF And Not rsAmpliado.BOF
                            bResultLiquidacion = False
                            bResultLiquidacion = oCreditoTmp.VerificaActualizacionLiquidacion(rsAmpliado("cCtaCod"))
                            If (Not bResultLiquidacion) Then
                                
                                Set oCreditoTmp = Nothing
                                rsAmpliado.MoveFirst
                                MsgBox "El crédito " & rsAmpliado("cCtaCod") & " no tiene actualizados sus datos de liquidación " & _
                                "no podrá realizar la ampliación a menos que actualice estos datos. Deberá comunicarse con el área de T.I.", vbExclamation, "Aviso"
    ...
    If bRefinanciar Then
        If IsArray(MatCredRef) Then
            If UBound(MatCredRef) > 0 Then
                ...
                For i = 0 To UBound(MatCredRef) - 1
                    bResultLiquidacion = False
                    bResultLiquidacion = oCreditoTmp.VerificaActualizacionLiquidacion(MatCredRef(i, 0))
                    If (Not bResultLiquidacion) Then
                    
                        Set oCreditoTmp = Nothing
                        MsgBox "El crédito " & MatCredRef(i, 0) & " no tiene actualizados sus datos de liquidación " & _
                        "no podrá realizar la refinanciación a menos que actualice estos datos. Deberá comunicarse con el área de T.I.", vbExclamation, "Aviso"
...
'                    gdFecSis, gsCodUser, gsCodCMAC, gsCodAge, CInt(Mid(Trim(Right(cmbTipoCred.Text, 5)), 1, 1) & Right(cmbSubTipo.Text, 2)), CInt(Trim(Right(cmbMoneda.Text, 2))), fsCtaCod, sCargo, sCARBEN, sT_Plani, CInt(Trim(Right(cmbMotivoRef.Text, 5))))
            ...
            Call oCredito.NuevaSolicitud(MatCredRelaciones, psNuevaCta, Trim(Right(cmbCondicion.Text, 5)), _
                    ...
                    nSubDestino, nTpDoc, nTpIngr, nTpInt, fvListaCreditoVerde, lvTablasRSE) 'EAAS 20180807 SEGUN ERS-054-2018 fvListaAguaSaneamiento
'JOEP20190919 ERS042 CP-2018 nSubDestino, nTpDoc, nTpIngr,nTpInt
'EAAS20191401 SEGUN 018-GM-DI_CMACM fvListaCreditoVerde
             'agregado por vapi SEGÙN ERS TI-ERS001-2017
            'CTI3 ERS0032020***********************************************
            If lvTablasRSE(1) = "NO" Then
                MsgBox "Falta general valores en las tabla " & lvTablasRSE(2) & ". Contactarse con el área de TI", vbInformation, "Aviso"
                Exit Sub
            End If
            'Dim oDCreditos As COMDCredito.DCOMCreditos
            Set oDCreditos = New COMDCredito.DCOMCreditos
            oDCreditos.ReclasificaCategoriaCredito (psNuevaCta)
            Set oDCreditos = Nothing
            '**************************************************************
...
'           ALPA 20100604 B2************************************************************************
            ...
            Call oCredito.ActualizaSolicitud(MatCredRelaciones, psNuevaCta, Trim(Right(cmbCondicion.Text, 5)), _
                     Trim(Right(cmbCondicionOtra.Text, 5)), CDbl(txtMontoSol.Text), CInt(spnCuotas.valor), CInt(spnPlazo.valor), Trim(Right(cmbDestCred.Text, 5)), Trim(Right(cmbAnalista.Text, 15)), _
                    sCodInstitucion, sCodModular, gdFecSis, sMovAct, bRefinanciarSustituir, CInt(Trim(Right(IIf(Trim(LblCondProd.Caption) = "", "0", LblCondProd.Caption), 5))), MatCredRefSust, IIf(ChkCap.value = 1, True, False), , nCampanaCod, _
                    gdFecSis, gsCodUser, gsCodCMAC, gsCodAge, fsCtaCod, sCargo, sCARBEN, sT_Plani, CInt(Trim(Right(IIf(cmbMotivoRef.Text = "", 0, cmbMotivoRef.Text), 5))), CInt(Mid(Trim(Right(cmbProductoCMACM.Text, 5)), 1, 1) & Right(cmbSubProducto.Text, 2)), nCasaCod, Me.txtVendedor.Text, Trim(txtDetalleMotivoRef.Text), IIf(fraPromotor.Visible And cmbPromotor.ListIndex <> -1, fbRegPromotores, False), Trim(Right(cmbPromotor.Text, 15)), lbEliminaCobertura, fvListaCompraDeuda, fnMontoExpEsteCred_NEW, fbEliminarEvaluacion, fvListaAguaSaneamiento, nSubDestino, nTpDoc, nTpIngr, nTpInt, fvListaCreditoVerde, lvTablasRSE) 'CInt(Trim(Right(cmbMotivoRef.Text, 5))))
...
End Sub
'CTI3 ERS0032020****************************************************************
Private Sub CP_CargaTpTipoIngreso(cTpProd As String, nParCod As Long)
Dim oDCred As COMDCredito.DCOMCredito
Dim rsTpDoc As ADODB.Recordset
Dim rsCrViPar As ADODB.Recordset
Dim lsPersCodTitular As String
Set oDCred = New COMDCredito.DCOMCredito
cmbTpIngreso.Enabled = True
Set rsTpDoc = oDCred.RSE_ObtieneTpDoc(cTpProd, nParCod)
    If Not (rsTpDoc.BOF And rsTpDoc.EOF) Then
        lblTipoIngreso.Caption = "Tipo de Ingresos"
        nTpCmbTpIng = 1
        Call Llenar_Combo_con_Recordset(rsTpDoc, cmbTpIngreso)
        Call CambiaTamañoCombo(cmbTpIngreso, 220)
        If Trim(Right(cmbSubProducto.Text, 10)) = "525" Then
            lsPersCodTitular = oRelPersCred.ObtenerMatrizRelacionesRelacion(20, 1)
            Set rsCrViPar = oDCred.RSE_ObtieneCreditoVigentexParalelo(lsPersCodTitular)
            
           If Not (rsCrViPar.BOF And rsCrViPar.EOF) Then
                If rsCrViPar!nTpIngr = -1 Then
                    cmbTpIngreso.Enabled = True
                Else
                    cmbTpIngreso.Enabled = False
                    cmbTpIngreso.ListIndex = IndiceListaCombo(cmbTpIngreso, IIf(rsCrViPar!nTpIngr = -1, 0, rsCrViPar!nTpIngr))
                End If
           Else
                MsgBox "El cliente no tiene un crédito vigente.", vbInformation, "Aviso"
                Exit Sub
           End If
        End If
    Else
        Call Llenar_Combo_con_Recordset(rsTpDoc, cmbTpIngreso)
        Call CambiaTamañoCombo(cmbTpIngreso, 220)
        cmbTpIngreso.ListIndex = -1
        
        sT_Plani = ""
        nTpCmbTpIng = 0 '0:Nada - Para identificar el tipo de Combo cmbTpDoc
    End If
Set oDCred = Nothing
RSClose rsTpDoc
RSClose rsCrViPar ' CTI3 ERS0032020
End Sub
'********************************************************************************
...
'                        txtMontoSol.Text = "0.00"
'                    End If
                    'MACM Segmantaci?n consumo experian 14052021
                    'Inicio MACM
                    If IsNull(rsMontoPre!m) Then
                        MsgBox "El cliente no se encuentra apto para la campaña", vbInformation, "Aviso"
                        cmdCancela_Click
...
'CTI3 ERS0032020
'If nTpMsn = 3 Then
If nTpMsn = 3 And Trim(Right(cmbSubProducto.Text, 5)) = 525 Then
    If (cmbTpDoc.Visible = True And cmbTpDoc.Text = "" And cmbTpDoc.Enabled = True) Then
...
If Right(Trim(cmbSubProducto.Text), 3) <> "703" Then
    If nTpMsn = 8 Then
        If cmbTpIngreso.Text = "" And nTpCmbTpIng <> 0 Then
            MsgBox "Seleccione el Tipo de Ingreso.", vbInformation, "Aviso"
            If cmbTpIngreso.Enabled = True Then
                EnfocaControl cmbTpIngreso
            End If
