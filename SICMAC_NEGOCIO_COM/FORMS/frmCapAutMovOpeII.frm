...
Begin VB.Form frmCapAutMovOpe 
   ...
   ClientTop       =   330
   ClientWidth     =   15270
   Icon            =   "frmCapAutMovOpeII.frx":0000
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ...
   Begin SICMACT.FlexEdit FlexData 
      ...
      EncabezadosAnchos=   "300-4000-2300-5700-1200-1200-800-3000-0-1400-0-0"
...
'  ssql = "exec Cap_MuestaMovOpePendientes '" & Vusuario & "'"
  ' Opera,Cuenta, persona,m.nmontosolicitado,m.nmontoaprobado,moneda,m.cautobs,m.cautestado,m.nidaut
    ...
    Set rs = oCap.ObtenerListaCuentas(gsCodUser)
...
If Trim(FlexData.TextMatrix(1, 1)) = "" Then
    MsgBox "No existen datos para Aprobar", vbInformation, "Aviso"
    Exit Sub
End If
If FlexData.Rows - 1 > 0 Then
  Dim vFilas As Long
        vFilas = FlexData.Rows - 1
  ...
  End If
    ...
    If vFilas > FlexData.Row Then
       FlexData.lbPuntero = False
       FlexData.Row = FlexData.Row + 1
       lblEstado.Caption = IIf(FlexData.TextMatrix(FlexData.Row, 8) = "P", "Pendiente", IIf(FlexData.TextMatrix(FlexData.Row, 8) = "A", "Aprobada", "Rechazada"))
       FlexData.lbPuntero = True
       FlexData.SetFocus
...
Private Sub cmdGrabar_Click()
 Dim aut As COMDCaptaGenerales.COMDCaptAutorizacion, rs As ADODB.Recordset, herror As Boolean, VUltAct As String
 ...
 Set aut = New COMDCaptaGenerales.COMDCaptAutorizacion
 herror = False
            
  Dim cnromov As New COMDMov.DCOMMov
  Dim Valor As String
  Valor = cnromov.GeneraMovNro(Format(Date, "yyyy-mm-dd hh:mm:ss"), "01", Vusuario)
    ...
    i = 1
        ...
        Do While i <= (FlexData.Rows - 1)
         ...
         End With
            herror = IIf(aut.AMovAutorizacionApro(nIdAut, cCtaCod, cPersCod, cOpeCod, "280715", nMontoSolicitado, nMontoAprobado, nMoneda, cautestado, cAutObs, "", Date, Vusuario, Date, "", VUltAct), herror, True)
            i = i + 1
        Loop
