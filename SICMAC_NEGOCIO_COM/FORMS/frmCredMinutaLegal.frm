VERSION 5.00
Begin VB.Form frmCredMinutaLegal 
   BorderStyle     =   3  'Fixed Dialog
   ClientHeight    =   7230
   ClientLeft      =   105
   ClientTop       =   435
   ClientWidth     =   12795
   Icon            =   "frmCredMinutaLegal.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   7230
   ...
   End
End
Attribute VB_Name = "frmCredMinutaLegal"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'********************************************************************
'*** Nombre : frmCredMinutaLegal
...
Public Sub Inicio(ByVal pnTipEstado As Integer, ByVal pnDescTitulo As String, Optional ByVal pnTipoReg As TipoRegControl = gTpoRegCtrlMinutaLegal)
...
Private Sub cmdIngExp_Click()
    ...
    Call oNCOMColocEval.insEstadosExpediente(FECredSugAprob.TextMatrix(FECredSugAprob.row, 2), "En Asesoria Legal", lcMovNro, "", "", "", 1, 2002, lnTipoReg)
    MsgBox "El Expediente Ingresó a Asesoría Legal", vbInformation, "Aviso"
...
Private Sub cmdRIngObs_Click()
    ...
    Call oNCOMColocEval.insEstadosExpediente(FECredSugAprob.TextMatrix(FECredSugAprob.row, 2), "En Asesoria Legal", "", "", lcMovNro, "", 1, 2002, lnTipoReg)
    MsgBox "Re Ingreso de Expediente a Asesoría Legal", vbInformation, "Aviso"
    Me.cmdRIngObs.Enabled = False
    Me.cmdIngExp.Enabled = False
    Me.cmdSalObs.Enabled = False
    Me.cmdSalExp.Enabled = True
...
Private Sub cmdSalExp_Click()
    ...
    lcMovNro = oNCOMContFunciones.GeneraMovNro(gdFecSis, Right(gsCodAge, 2), gsCodUser)
    Call oNCOMColocEval.updateEstadoExpediente(FECredSugAprob.TextMatrix(FECredSugAprob.row, 2), lnTipoReg) 'BY ARLO MODIFY 20171027
    Call oNCOMColocEval.insEstadosExpediente(FECredSugAprob.TextMatrix(FECredSugAprob.row, 2), "Analista de Credito", "", "", "", lcMovNro, 2, 2002, lnTipoReg)
    MsgBox "El Expediente Salió de Asesoría Legal", vbInformation, "Aviso"
...
Private Sub cmdSalObs_Click()
    ...
    lcMovNro = oNCOMContFunciones.GeneraMovNro(gdFecSis, Right(gsCodAge, 2), gsCodUser)
    Call oNCOMColocEval.updateEstadoExpediente(FECredSugAprob.TextMatrix(FECredSugAprob.row, 2), lnTipoReg) 'BY ARLO MODIFY 20171027
    Call oNCOMColocEval.insEstadosExpediente(FECredSugAprob.TextMatrix(FECredSugAprob.row, 2), "Analista de Creditos", "", lcMovNro, "", "", 1, 2002, lnTipoReg)
    MsgBox "El Expediente Salió por Observación de Asesoría Legal", vbInformation, "Aviso"
...
Private Sub cargarCreditosAprobadosMinuta(ByVal pnTpoRegCtrl As TipoRegControl, ByVal psCtaCod As String, ByVal psTitular As String, ByVal psAgeCod As String)
    Set oNCOMColocEval = New NCOMColocEval
    Set rsCredSugAprob = New ADODB.Recordset
    Dim I As Integer
    ...
    Set rsCredSugAprob = oNCOMColocEval.obtenerCreditosAprobadosMinuta(pnTpoRegCtrl, psCtaCod, psTitular, psAgeCod)
...
Private Sub CargarDatos()
    If optCredito.value = True Then
        Call cargarCreditosAprobadosMinuta(lnTipoReg, ActXCodCta.NroCuenta, "", "")
    ElseIf optTit.value = True Then
        Call cargarCreditosAprobadosMinuta(lnTipoReg, "", txtTitular.Text, "")
    Else
        Call cargarCreditosAprobadosMinuta(lnTipoReg, "", "", Mid(cboAgencia.Text, Len(cboAgencia.Text) - 1, 2))
    End If
End Sub
