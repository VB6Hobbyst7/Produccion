...
Private Sub cmdGrabar_Click()
    ...
    If fbNuevo Then
        lnIDCheque = oDR.RegistrarCheque(gsCodArea, Right(gsCodAge, 2), gsCodUser, txtNroCheque.Text, txtNroCheque1.Text, txtNroCheque2.Text, txtNroCheque3.Text, txtNroChequeCtaIF.Text, txtNroCheque4.Text, _
                            ...
                            ldDepFechaReg, lsDepIFTpo, lsDepPersCod, lsDepCtaIF, lnMovNroPend, lMatMovNro, lsCtaContRegistroD, lsCtaContRegistroH, lsCtaContDepositoD, lsCtaContDepositoH)
    Else
        lnIDCheque = oDR.EditarCheque(fnId, txtGiradorCod.Text, txtContactoCod.Text, fMatOperaciones)
    End If
    Screen.MousePointer = 0
...
Private Sub cargarControles()
    cboMoneda.Clear
    cboMoneda.AddItem "S/." & Space(100) & "1"
...
Private Sub txtNroChequeCtaIF_KeyPress(KeyAscii As Integer)
    ...
    If KeyAscii = 13 Then
        If Len(txtNroChequeCtaIF.Text) > 0 Then
            Set oDR = New COMNCajaGeneral.NCOMDocRec
            'PASI20140619
            If oDR.ObtieneExisteDocCheque(txtNroCheque.Text, fsIFICod, fsIFITpo, txtNroChequeCtaIF.Text) Then
...
Private Sub feOperaciones_RowColChange()
    ...
    If feOperaciones.lbEditarFlex Then
        If feOperaciones.col = 2 Then
            Set rs = oConst.RecuperaConstantes(10034)
...
Private Sub HabilitaxEditar(ByVal pbHabilita As Boolean)
    ...
    txtImporte.Locked = Not pbHabilita
...
Public Sub ImprimeConstanciaPDF(ByVal pnId As Long)
    ...
    oPDF.WTextBox 174, 40, 10, 515, "2. DATOS DEL SOLICITANTE", "F2", 8, hLeft, , vbWhite, 1, vbRed, True
