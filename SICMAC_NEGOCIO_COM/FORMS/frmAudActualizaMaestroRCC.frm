...
Do While Not EOF(1)   ' Repite el bucle hasta el final del archivo.
    ...
    If Len(Trim(lsLinea)) > 0 Then ' Linea tiene datos
        If Mid(lsLinea, 1, 1) = "1" Then ' Datos de cliente
            ...
            lsSQL = "INSERT INTO " & gcServBDConsol & "RCC ( cCodSBS, cTiDoTr, cNuDoTr, cTiDoCi, cNuDoCi, " _
...
End Sub
Private Function VerificaMaestro(ByVal psCodDeu As String) As Boolean
Dim sql As String
Dim rs1 As New ADODB.Recordset
'dbCmact.Open gsConnection
'dbCmact.Close
sql = "Select cCodSBS From " & gcServerCentralR & "RCC Where cCodSBS ='" & psCodDeu & "'"
rs1.Open sql, dbCmact, adOpenStatic, adLockReadOnly, adCmdText
If Not RSVacio(rs1) Then
    VerificaMaestro = True
Else
    VerificaMaestro = False
End If
rs1.Close
Set rs1 = Nothing
End Function
...
Do While Not EOF(1)   ' Repite el bucle hasta el final del archivo.
    ...
    If Len(Trim(lsLinea)) > 0 Then ' Linea tiene datos
        'If Mid(lsLinea, 1, 1) = "1" Then ' Datos de cliente
            ...
            lsSQL = "INSERT INTO " & gcServBDConsol & "ICS (cTIDOID, cNUDOID, cTIPPERS, " _
...
'dbCmact.Open gsConnection
'dbCmact.Close
sql = "Select cTipPers From " & gcServBDConsol & "ICS Where cTIDOID ='" & psTiDoID & "' AND " _
    & " rtrim(ltrim(cNUDOID)) = '" & psNuDoId & "' "

rs1.Open sql, dbCmact, adOpenStatic, adLockReadOnly, adCmdText
If Not RSVacio(rs1) Then
    VerificaMaestroICS = True
Else
    VerificaMaestroICS = False
End If
rs1.Close
Set rs1 = Nothing
End Function
