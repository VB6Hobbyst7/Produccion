...
Begin VB.Form frmCredBPPTipoCredNiveles 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "BPP - Tipo Creditos X Niveles"
   ClientHeight    =   6675
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   7695
   Icon            =   "frmCredBPPTipoCredNiveles.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   6675
   ...
   Begin TabDlg.SSTab SSTNivelesCartera 
      ...
      TabPicture(0)   =   "frmCredBPPTipoCredNiveles.frx":030A
      ...
      Begin VB.Frame fraEditarNivel 
         ...
         Top             =   360
         Width           =   7215
         Begin SICMACT.FlexEdit feSubProductos 
            Height          =   5055
            Left            =   120
            TabIndex        =   2
            Top             =   240
            ...
            HighLight       =   1
            AllowUserResizing=   3
            EncabezadosNombres=   "Nº-TipoCredito-Nivel-CodTpoProd-Aux-NivelAnt"
            EncabezadosAnchos=   "500-4000-2000-0-0-0"
   ...
   End
End
Attribute VB_Name = "frmCredBPPTipoCredNiveles"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'*****************************************************************************************
'***     Rutina         :   frmCredBPPTipoCredNiveles
...
If MsgBox("Estas seguro de Guardar?", vbInformation + vbYesNo, "Aviso") = vbYes Then
    ...
    For i = 0 To feSubProductos.Rows - 2
        If CInt(Trim(Right(feSubProductos.TextMatrix(i + 1, 2), 4))) <> CInt(Trim(feSubProductos.TextMatrix(i + 1, 5))) Then
            Call oBPP.OpeSubProductoscredNiveles(CInt(feSubProductos.TextMatrix(i + 1, 4)), feSubProductos.TextMatrix(i + 1, 3), CInt(Trim(Right(feSubProductos.TextMatrix(i + 1, 2), 4))), 2)
        End If
    Next i
    Call CargaControles
...
'Set rsSubProd = oCredito.RecuperaTipoCreditos
Set rsSubProd = oCredito.RecuperaSubTipoCrediticios
If Not (rsSubProd.EOF And rsSubProd.BOF) Then
    For i = 0 To rsSubProd.RecordCount - 1
        ...
        Set rsBPP = oBPP.ObtenerNivelesXSubProdcred(UCase(Trim(rsSubProd!nConsValor)), 2)
