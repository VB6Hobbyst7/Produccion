VERSION 5.00
Begin VB.Form frmCapFideicomiso 
   ...
   ClientTop       =   330
   ClientWidth     =   7695
   Icon            =   "frmCapFideicomiso.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   Moveable        =   0   'False
   ...
   End
End
Attribute VB_Name = "frmCapFideicomiso"
...
Private Sub cmdBuscar_Click()
    ...
    LstCred.Clear
    frmBuscaPersFideicomiso.Inicio
...
If MsgBox("Se va a Efectuar el Pago del Fideicomiso, Desea Continuar ?", vbInformation + vbYesNo, "Aviso") = vbYes Then
    ...
    oGraba.CapPagoFideicomiso lsCodCta, PlanPagos, Val(txtTotal.Text), lnCuotasPag, cboTipoPago.ListIndex, lsMovNro, lblCalendario, lnCuotaPagPJ

    lsBoleta = oGraba.ImprimeBoleta(lsCodCta, lblNombre.Caption, gsNomAge, lblMoneda, lblCuotasPag, gdFecSis, Time, lnTCapital, lnTInteres, _
                                    lnTPortes, lnTIntComp, lnTMora, lnTComVcdo, lnTProtesto, lnTGastos, gsCodUser, sLpt, gsCodCMAC)
...
Private Sub BuscaFideicomiso(ByVal psCodCta As String)
Dim oDatos As COMDCaptaServicios.DCOMCaptaFideicomiso
Dim rs As Recordset
Dim lnInteres As Currency
Dim lnGastos As Currency
...
Set oDatos = New COMDCaptaServicios.DCOMCaptaFideicomiso

Set rs = oDatos.dGetDatosFideicomiso(psCodCta)
...
Set rs = oDatos.dGetDatosFideicomisoDet(psCodCta)
...
Private Sub txtTotal_KeyPress(KeyAscii As Integer)
    ...
    If KeyAscii = 13 Then
        If txtTotal <> "" Then
            ...
            If CCur(txtTotal) > CCur(lblDeuda) Then
                MsgBox "El Monto a Pagar no puede ser mayor al valor de la deuda", vbInformation, "Aviso"
                txtTotal.SetFocus
                Exit Sub
            Else
                cmdGrabar.SetFocus
