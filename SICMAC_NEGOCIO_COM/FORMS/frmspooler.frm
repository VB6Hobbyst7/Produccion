...
Begin VB.Form frmSpooler 
   ...
   Begin VB.Frame fraZoom 
      ...
      Begin VB.ComboBox cboPrevio 
         ...
         EndProperty
         Height          =   315
         ItemData        =   "frmspooler.frx":1B12
         Left            =   120
         List            =   "frmspooler.frx":1B2E
    ...
    Case "imprimir"
        If lstArch.ListItems.Count <> 0 Then
            ...
            If chkPaginas.value = 1 Then
                If Posi <> 0 Then
                    tmpText = Chr(12) & Mid(rtfAux.Text, 1, Posi - 1)
                Else
                    tmpText = Chr(12) & rtfAux.Text
                End If
                X = 0: Y = 0
                vPosIni = 0: vPosFin = 0
                Do While True
                    X = X + 1
                    vPosIni = InStr(vPosIni + 1, tmpText, Chr(12), vbTextCompare)
                    If X > Val(txtPagIni.Text) - 1 Then Exit Do
                Loop
                Do While True
                    Y = Y + 1
                    vPosFin = InStr(vPosFin + 1, tmpText, Chr(12), vbTextCompare)
...
If lstArch.ListItems.Count <> 0 Then
    ...
    vRelle = String(Round(rtfView.RightMargin / 155), "»") & Chr(10)
    vPage = 0
    ...
    Do While True And Len(Trim(vTexto)) > 0
       vPage = vPage + 1
       N = InStr(1, vTexto, Chr(12))
       If N > 0 Then
          sPag = sPag & Mid(vTexto, 1, N - 1) & Chr(10) & Chr(10) & "<<Página :" & ImpreFormat(vPage, 7, 0) & ">>" & Chr(10) & vRelle
          vTexto = Mid(vTexto, N + 1)
       Else
           sPag = sPag & vTexto & Chr(10) & Chr(10) & "<<Página :" & ImpreFormat(vPage, 7, 0) & ">>" & Chr(10)
           Exit Do
       End If
    ...
    '    vPage = vPage + 1
    '    If InStr(1, rtfView.Text, Chr(12)) > 0 Then
    '        rtfView.Text = Replace(rtfView.Text, Chr(12), Chr(10) & Chr(10) & "<<Página :" & ImpreFormat(vPage, 7, 0) & ">>" & Chr(10) & vRelle, 1, 1, vbTextCompare)
    '    Else
    '        rtfView.Text = rtfView.Text & Chr(10) & Chr(10) & "<<Página :" & ImpreFormat(vPage, 7, 0) & ">>" & Chr(10)
    '        Exit Do
    '    End If
    'Loop
