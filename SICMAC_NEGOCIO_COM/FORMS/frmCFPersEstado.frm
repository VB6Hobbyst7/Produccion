...
Begin VB.Form frmCFPersEstado 
   ...
   End
End
Attribute VB_Name = "frmCFPersEstado"
...
Private Sub CargaGrid(ByVal pnTipoCredPers As Variant, Optional ByVal pMatProd As Variant = Nothing, Optional ByVal pind As Integer = 0, Optional ByVal pbComision As Boolean = False) 'WIOR 20130313 AGREGO pbComision
Dim oCreditos As COMNCartaFianza.NCOMCartaFianzaValida 'NCartaFianzaValida
    On Error GoTo ERRORCargaGrid
    Set oCreditos = New COMNCartaFianza.NCOMCartaFianzaValida
    'madm 20120117 - pind
    If pind = 1 Then
        Set RGrid = oCreditos.RecuperaPersonasEstadoCF(pnTipoCredPers, pMatProd, gdFecSis, pind)
    'WIOR 20120806*****
    ElseIf pind = 2 Then
            Set RGrid = oCreditos.RecuperaPersonasEstadoCF(pnTipoCredPers, pMatProd, gdFecSis, pind)
    ...
    'WIOR FIN *********
    Else
        Set RGrid = oCreditos.RecuperaPersonasEstadoCF(pnTipoCredPers, pMatProd, , , , pbComision, gdFecSis) 'WIOR 20130313 AGREGO gdFecSis
    End If
...
Public Function Inicio(ByVal pnTipoCredPers As Variant, ByVal psCaption As String, _
    Optional ByVal pMatProd As Variant = Nothing, Optional ByVal pind As Integer = 0, Optional ByVal pbComision As Boolean = False) As String 'WIOR 20130313 AGREGO pbComision
Dim i As Integer
    vTipoBusqueda = 1
    Me.Caption = psCaption
    vsSelecPers = ""
    Call CargaGrid(pnTipoCredPers, pMatProd, pind, pbComision)
    Screen.MousePointer = 0
    Me.Show 1
    Set RGrid = Nothing
