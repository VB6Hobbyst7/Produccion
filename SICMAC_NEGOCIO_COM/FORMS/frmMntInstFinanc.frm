...
Option Explicit
Dim oIFinan As COMDPersona.DCOMInstFinac
...
Set rs = oCons.RecuperaConstantes(gCGTipoIF)
Set oCons = Nothing
Call Llenar_Combo_con_Recordset(rs, CboTipoInstFinanc)

End Sub
Private Sub CargaDatos()

    Set oIFinan = New COMDPersona.DCOMInstFinac
...
Dim oNInstFin As COMNPersona.NCOMInstFinac
Dim sError As String

Dim sAgenteBolsa As String
...
On Error GoTo ErrorAceptar
    ...
    sError = ""
    Set oNInstFin = New COMNPersona.NCOMInstFinac
    sAgenteBolsa = oNInstFin.BuscarAgenteBolsa(Trim(TxtBPersInstFinanc.Text))
    
    If Trim(Right(CboTipoInstFinanc.Text, 15)) = "9" Then
        If sAgenteBolsa <> "10" Then
            MsgBox "Usted eligió un Sociedad Agente de Bolsa, la persona debe ser Agente de Bolsa", vbCritical
            Exit Sub
        End If
    End If
