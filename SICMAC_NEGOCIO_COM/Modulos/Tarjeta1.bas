...
'FUNCIONES PARA PINPAD ACS CAJA MAYNAS
'**********************************************************************************
'============================================================
'Declarando las funciones del PinPad
'============================================================

    '============================================================
    'Funciones para Leer Tarjeta de Banda y Simulada Smartcard
    '============================================================
    
    Public Declare Function ReadMagneticCard Lib "PinPad.dll" (ByVal I_MIN As Integer, ByVal I_TRACK As Integer) As Integer
                                                                
    Public Declare Function ReadTrack Lib "PinPad.dll" (ByVal I_NUMBER As Integer, ByVal i As Integer, ByVal f As Integer) As Long
    

    '============================================================
    'Funciones de Inicializacion / Finalizacion de comunicacion con el Pinpad
    '============================================================
    Public Declare Function OpenReader Lib "PinPad.dll" () As Integer
    Public Declare Sub CloseReader Lib "PinPad.dll" ()
    
    '============================================================
    'Display & Buzzer
    '============================================================
    Public Declare Sub LightOn Lib "PinPad.dll" (ByVal i As Integer)
    Public Declare Sub BuzzerOn Lib "PinPad.dll" (ByVal i As Integer)
    Public Declare Sub ClearDisplay Lib "PinPad.dll" ()
    Public Declare Sub DisplayString Lib "PinPad.dll" (ByVal I_LINE As Integer, ByVal I_OFFSET As Integer, ByVal I_TYPE As Integer, ByVal s As String)
    
    '============================================================
    'KeyBoard
    '============================================================
    Public Declare Function ReadKeyPinPad Lib "PinPad.dll" (ByVal I_TIMEOUT As Integer) As Integer
    
    '============================================================
    'PINValidate with PINVerify
    '============================================================
    Public Declare Function CheckPIN Lib "PinPad.dll" (ByVal I_TIMEOUT As Integer, ByVal S_IPSERVER As String, ByVal S_PAN As String, ByVal S_PVKI As String, ByVal S_PVV As String) As Integer
    ' Public Declare Function ChangePIN Lib "PinPad.dll" (ByVal I_TIMEOUT As Integer, ByVal S_IPSERVER As String, ByVal S_PAN As String, ByVal S_PVKI As String, ByVal S_PVV As String, ByVal S_MSG As String) As Long

    Public Declare Function ChangePIN Lib "PinPad.dll" (ByVal I_TIMEOUT As Integer, ByVal S_IPSERVER As String, ByVal S_PAN As String, ByVal S_PVKI As String, ByVal I_CHNG_PASS As Integer, S_MSG As String) As Long
    
'============================================================
'Otras funciones
'============================================================
    Public Declare Function lstrlenA Lib "kernel32" (ByVal lpString As Long) As Long
    Public Declare Function SysAllocStringByteLen Lib "oleaut32" (ByVal ansistr As Long, ByVal BLen As Long) As String
    
    
'============================================================
' Declarar Variables
'============================================================
    Public I_MIN       As Integer
    Public I_TRACK     As Integer
    Public I_NUMBER    As Integer
    Public I_LINE      As Integer
    Public I_OFFSET    As Integer
    Public I_TYPE      As Integer
    Public I_TIMEOUT   As Integer
    Public I_LONG_MAX  As Integer
    Public S_IPSERVER  As String
    Public S_PAN       As String
    Public S_PVKI      As String
    Public S_PVV       As String
    Public S_PIN       As String
    Public S_MSG       As String
    Public S_S         As String
    Public I_CHNG_PASS As String
    Public s_Error     As String
    Public s_LogFile   As String

Public Sub CargaValoresPinPad_ACS()
    '============================================================
    ' Declarar Variables
    '============================================================
    I_MIN = 60
...
'**********************************************************************************
Public Function sANSIPtrToVBString(lStrptr As Long, lNChars As Long) As String
...
Function WriteToLcd(szMessage$) As Boolean
    ...
    Select Case gnTipoPinPad
        ...
        Case gPerifPINPAdVrfSC552  'VERIFONE SC 552
            WriteToLcd = True  'WriteToLcd_VrfSC552(szMessage)
...
Function WriteToLcd_Vrf5000(szMessage$) As Boolean
    ...
    'lalgo = GmyPSerial.ReadCardIniConf(szMessage)
     ...
     GmyPSerial.ShowMessage (szMessage)
    'If GmyPSerial.ReadCardIniConf(szMessage) Then
    '    lbResult = True
    'Else
    '    lbResult = False
    'End If
    
    WriteToLcd_Vrf5000 = True
...
End Function
Public Function GetNumTarjeta_ACS() As String
...
'            Exit Function
'        End If
'        If ObjTarj.VerificaTarjetaActivaReg(cTarjeta) Then
'            MsgBox "Tarjeta Invalida, para Registro", vbInformation, "AVISO"
'            CloseReader
'            Set ObjTarj = Nothing
'            Exit Function
'        End If
'        GetNumTarjeta_ACS = Mid(S_RESULT, 3, 16)
'
'    End If
'    CloseReader
End Function
...
Public Function GetClaveTarjeta(Optional psCadena As String = "") As String
    ...
    Select Case gnTipoPinPad
        ...
        Case gPerifPINPAdVrfSC552  'VERIFONE SC 552
            GetClaveTarjeta = GetClaveTarjeta_VrfSC552(psCadena) 'psNumTarj)
        Case gPerifPINPAdHiperCom  'HIPERCOM
...
'    On Error Resume Next
    ...
    GmyPSerial.ReadPinIniConf "0", "1111111111111111", "0000000000000000", "INGRESE CLAVE "
...
Private Function ObtieneTipoPinPad() As TipoPinPad
    ...
    Set oCons = New COMDConstantes.DCOMConstantes
        Set lrst = oCons.ObtieneTipoPinPad(lsNombrePc, lnPeriferico)
    Set oCons = Nothing
...
'        GmyPSerial.Disconnect
'    End If
    'If myPSerial.ConnectionTest = 0 Then Call myPSerial.Connect(CInt(IIf(GetCOM("PINPAD") = "COM1", "1", "2")), 9600)
'    Set GmyPSerial = CreateObject("HComPinpad.Pinpad")
    'GmyPSerial.Reset
    Call GmyPSerial.Connect(CInt(nPuerto), 9600)
    If GmyPSerial.Connect(CInt(nPuerto), 9600) = 0 Then
        IniciaPinPad_Vrf5000 = True
    Else
        IniciaPinPad_Vrf5000 = False
    End If
...
'ARCV : Comentar para compilar Clases
Public Function GetClaveTarjeta_ACS(ByVal psTarjeta As String, Optional ByVal nlee As Integer = 0) As Integer
Dim ObjTarj As COMNCaptaServicios.NCOMCaptaTarjeta
Set ObjTarj = New COMNCaptaServicios.NCOMCaptaTarjeta
'VerificaTarjetaActivaReg
Dim I_RESULT As Integer
...
I_RESULT = OpenReader()
S_RESULT = CStr(I_RESULT)
GetClaveTarjeta_ACS = 0
'I_RESULT = ReadMagneticCard(I_MIN, I_TRACK)
If nlee = 0 Then I_RESULT = ReadMagneticCard(60, 1)
S_RESULT = CStr(I_RESULT)
If I_RESULT > 0 Then
    S_PAN = psTarjeta
    S_PVKI = gsPVKi
    ...
    CloseReader
    GetClaveTarjeta_ACS = I_RESULT
Else
    MsgBox "No hay Conexion con el PinPad", vbInformation, "AVISO"
End If
Set ObjTarj = Nothing
End Function
