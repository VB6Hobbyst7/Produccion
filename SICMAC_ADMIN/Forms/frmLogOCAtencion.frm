...
Public Sub InicioRep(pbBienes As Boolean, psOpeCod As String, Optional pbPresu As Boolean = False, Optional pbModifica As Boolean = False, Optional pbImprime As Boolean = False, Optional pbRegComp As Boolean = False) 'WIOR 20130110 agrego pbRegComp
...
Public Sub Inicio(pbBienes As Boolean, psOpeCod As String, Optional pbPresu As Boolean = False, Optional pbModifica As Boolean = False, Optional pbImprime As Boolean = False, Optional pbRangoFec As Boolean = False, Optional pbRegComp As Boolean = False) 'WIOR 20130110 Agrego pbRegComp
...
If lbPresu Or lbModifica Or lbImprime Or lbRegComp Then 'WIOR 20130110 AGREGO lbRegComp
...
Do While Not rs.EOF
   ...
   If Not (lbPresu Or lbModifica Or lbImprime Or lbRegComp) Then 'WIOR 20130110 AGREGO lbRegComp
...
If fg.TextMatrix(2, 1) = "" Then
   Exit Sub
End If
If Not lbPresu And Not lbModifica And Not lbImprime And Not lbRegComp Then 'WIOR 20130110 AGREGO lbRegComp
   If fg.TextMatrix(fg.row, 10) = "Pendiente" Then
      MsgBox "O/C aún no aprobado por Presupuesto", vbInformation, "¡Aviso!"
      Exit Sub
   End If
End If
If Not lbImprime And Not lbRegComp Then 'WIOR 20130110 AGREGO lbRegComp
...
If lbPresu = False Then
   If lbModifica Or lbImprime Or lbRegComp Then 'WIOR 20130110 AGREGO lbRegComp
      Set frmLogOCompra = Nothing
      
      frmLogOCompra.Inicio True, lsOpeCod, fg.TextMatrix(fg.row, 9), lbBienes, True, lbImprime, fg.TextMatrix(fg.row, 11), , lbRegComp 'WIOR 20130110 AGREGO lbRegComp
...
Private Sub cmdActualizar_Click()
    'GetOCPendientes Right(cmbTipoOC.Text, 1) 'Comentado PASI20150915 ERS0472015
    
    'PASI20150915 ERS0472015
    If Not ValidaDatos Then Exit Sub
    If Not lbModifica Then
        GetOCPendientes Right(cmbTipoOC.Text, 1)
    Else
        GetOCPendientesNew Right(cmbTipoOC.Text, 1)
    End If
    'end PASI
End Sub
'PASI20150915 ERS0472015
Private Function ValidaDatos() As Boolean
    ValidaDatos = False
    If ValidaFecha(Me.mskIni.Text) <> "" Then
        MsgBox "¡ Fecha no Válida !", vbInformation, "¡Aviso!"
        mskIni.SetFocus
        Exit Function
    End If
    If ValidaFecha(Me.mskFin.Text) <> "" Then
       MsgBox "¡ Fecha no Válida !", vbInformation, "¡Aviso!"
       mskFin.SetFocus
       Exit Function
    End If
    If CDate(Me.mskIni.Text) > CDate(Me.mskFin.Text) Then
        MsgBox "¡ Fecha de Inicio no puede ser mayor a la Fecha de Fin!", vbInformation, "¡Aviso!"
        mskIni.SetFocus
        Exit Function
    End If
    ValidaDatos = True
...
'END PASI
'PASI20150915 ERS0472015
Private Sub GetOCPendientesNew(Optional STIPOoc As String = "")
    Dim lsCadSerAdm As String
Dim nItem As Integer
Dim nTot  As Currency
Dim oCon As DConecta
...
If lbPresu Or lbModifica Or lbImprime Or lbRegComp Then
        If STIPOoc = "T" Then
                    sSQL = " SELECT b.dDocFecha, g.cDocAbrev, b.nDocTpo, b.cDocNro,cdocnroOCD  = isnull((select cDocNro  from movdoc md where md.nMovNro = a.nMovNro and nDocTpo IN  (Select Valor From dbo.fnc_getTblValoresTexto('" & sTipDoc & "')) ),''),   dd.cPersCod, " _
                     & " TipoOC = isnull((select ctipoOC  from movcotizac mx where mx.nMovNro = a.nMovNro ),''),cNomPers = (" _
                     & "     SELECT cPersNombre + space(50) + PE.cPersCod " _
                     & "     FROM Persona PE" _
                     & "     WHERE PE.cPersCod = dd.cPersCod)" _
    ...
    If lbImprime Or lbRegComp Then
            If gbBitCentral Then
                    If STIPOoc = "T" Then
                        sSQL = "stp_sel_ERS0472015_GetOcPendienteNew '" & sTipDoc & "','" & Format(Me.mskIni.Text, "yyyyMMdd") & "','" & Format(Me.mskFin.Text, "yyyyMMdd") & "','" & sCtaProvis & "','" & sDocTpoOC & "','%'"
                    Else
                        sSQL = "stp_sel_ERS0472015_GetOcPendienteNew'" & sTipDoc & "','" & Format(Me.mskIni.Text, "yyyyMMdd") & "','" & Format(Me.mskFin.Text, "yyyyMMdd") & "','" & sCtaProvis & "','" & sDocTpoOC & "','" & STIPOoc & "'"
                    End If
            Else
                sSQL = " SELECT b.dDocFecha, g.cDocAbrev, b.nDocTpo, b.cDocNro, dd.cPersCod, " _
                     & " cNomPers = (" _
...
If lbReporteFechas Then
    If STIPOoc = "T" Then
        sSQL = "stp_sel_ERS0472015_GetOcPendienteNew '" & sTipDoc & "','" & Format(Me.mskFecIni.Text, "yyyyMMdd") & "','" & Format(Me.mskFecFin.Text, "yyyyMMdd") & "','" & sCtaProvis & "','" & sDocTpoOC & "','%'"
    Else
        sSQL = "stp_sel_ERS0472015_GetOcPendienteNew '" & sTipDoc & "','" & Format(Me.mskFecIni.Text, "yyyyMMdd") & "','" & Format(Me.mskFecFin.Text, "yyyyMMdd") & "','" & sCtaProvis & "','" & sDocTpoOC & "','" & STIPOoc & "'"
    End If
End If
...
Do While Not rs.EOF
   ...
   fg.TextMatrix(nItem, 7) = rs!cMovDesc
   fg.TextMatrix(nItem, 8) = rs!nMovNro
   fg.TextMatrix(nItem, 9) = Right(rs!cNomPers, 13) & ""
   If Not (lbPresu Or lbModifica Or lbImprime Or lbRegComp) Then
      fg.TextMatrix(nItem, 10) = Format(rs!nDocImporte - rs!nMontoA, gcFormView)
   Else
      fg.TextMatrix(nItem, 10) = Format(rs!nDocImporte, gcFormView)
   End If
...
If oMov.ExisteMovContratoOrden(fg.TextMatrix(fg.row, 8)) Then 'PASIERS0772014
    oMov.ActualizaMovContratoOrden (fg.TextMatrix(fg.row, 8))
End If
    'ARLO 20170125
    Dim lsPalabra As String
    If (lsOpeCod = 501206 Or lsOpeCod = 502206) Then
    lsPalabra = "Compra"
    ElseIf (lsOpeCod = 501208 Or lsOpeCod = 502208) Then
    lsPalabra = "Servicio"
    End If
    gsOpeCod = lsOpeCod
    Set objPista = New COMManejador.Pista
    objPista.InsertarPista gsOpeCod, glsMovNro, gsCodPersUser, GetMaquinaUsuario, "3", "Elimino Orden " & lsPalabra & " N° " & fg.TextMatrix(fg.row, 2)
...
Private Sub CmdImprimir_Click()
    ...
    gsOpeCod = lsOpeCod
    Set objPista = New COMManejador.Pista
    objPista.InsertarPista gsOpeCod, glsMovNro, gsCodPersUser, GetMaquinaUsuario, "", "Imprimio " & sDocDesc & " de la fecha " & mskFecIni.Text & " a " & mskFecFin.Text
    Set objPista = Nothing
    '***********
End Sub
...
Private Sub Form_Load()
    ...
    If lbRegComp Then
        cmdExtornar.Visible = False
        cmdAceptar.Caption = "&Comprobante"
    End If
    'WIOR FIN ******************************
    If lsOpeCod = 501221 Or lsOpeCod = 502221 Then
       '''Me.Caption = "Aprobación Orden Compra - Presupuesto" & Space(2) & "-  " & IIf(Mid(lsOpeCod, 3, 1) = 1, "SOLES", "DOLARES") 'MARG ERS044-2016
       Me.Caption = "Aprobación Orden Compra - Presupuesto" & Space(2) & "-  " & IIf(Mid(lsOpeCod, 3, 1) = 1, StrConv(gcPEN_PLURAL, vbUpperCase), "DOLARES") 'MARG ERS044-2016
    End If
    If lsOpeCod = 501222 Or lsOpeCod = 502222 Then
       '''Me.Caption = "Aprobación Orden Servicio - Presupuesto" & Space(2) & "-  " & IIf(Mid(lsOpeCod, 3, 1) = 1, "SOLES", "DOLARES") 'MARG ERS044-2016
       Me.Caption = "Aprobación Orden Servicio - Presupuesto" & Space(2) & "-  " & IIf(Mid(lsOpeCod, 3, 1) = 1, StrConv(gcPEN_PLURAL, vbUpperCase), "DOLARES") 'MARG ERS044-2016
    End If
End Sub
