...
oCon.CierraConexion
        ...
        lsPalabras = "AGOSTO"
        ElseIf (nMes = 9) Then
        lsPalabras = "SEPTIEMBRE"
        ElseIf (nMes = 10) Then
        lsPalabras = "OCTUBRE"
        ElseIf (nMes = 11) Then
        lsPalabras = "NOVIEMBRE"
        ElseIf (nMes = 12) Then
        lsPalabras = "DICIEMBRE"
        End If
        
        gsOpeCod = LogPistaReporteEstadistico
        Set objPista = New COMManejador.Pista
        objPista.InsertarPista gsOpeCod, glsMovNro, gsCodPersUser, GetMaquinaUsuario, "", "Genero el Reporte Estadistico del Mes " & lsPalabras & " del " & Left(cAnioMes, 4)
...
Select Case nBase
    Case 0
         ...
         " where  d.nDocTpo = " & nDocTpo & " AND a.cAreaDescripcion is not NULL and m.cMovMoneda='" & cMoneda & "') " & _
         "  from MOVCOTIZAC m inner join MovDoc d on m.nMovNro = d.nMovNro " & _
         "       left outer join Areas a on left(m.cAreaCod,3) = a.cAreaCod " & _
         " where d.nDocTpo = " & nDocTpo & " AND " & _
    ...
    Case 1
         ...
         "       left outer join Areas a on m.cAreaCod = a.cAreaCod " & _
         " where len(m.cAreaCod)=3 AND d.nDocTpo = " & nDocTpo & " and nMovImporte>0 and m.cMovMoneda = '" & cMoneda & "')  " & _
         "  from MOVCOTIZAC m inner join MovDoc d on m.nMovNro = d.nMovNro " & _
         "       INNER JOIN MovCta c on m.nMovNro = c.nMovNro " & _
         "       left outer join Areas a on m.cAreaCod = a.cAreaCod " & _
         " Where  Len(m.cAreaCod) = 3 And d.nDocTpo = " & nDocTpo & " And nMovImporte > 0 and m.cMovMoneda = '" & cMoneda & "'" & _
