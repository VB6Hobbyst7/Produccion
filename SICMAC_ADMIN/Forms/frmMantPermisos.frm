...
Dim R As ADODB.Recordset 'AMDO 20130717

Private Sub CargaTVewOperaciones(ByVal rsUsu As ADODB.Recordset) 'ADD rsUsu AMDO 20130717
'Dim rsUsu As Recordset 'AMDO 20130717
Dim sOperacion As String, sOpeCod As String
Dim sOpePadre As String, sOpeHijo As String, sOpeHijito As String
Dim nodOpe As Node
Dim N As Node

'Set rsUsu = oAcceso.DameRSOperaciones 'AMDO 20130717
sOpePadre = "P"
...
Private Function CargaAccesos(ByRef Node As Node, Optional ByVal pbOPeracion As Boolean = False) As Node 'Parametro add pbOPeracion, por AMDO 20130717
...
Dim L As ListItem
    ...
    For i = 1 To UBound(pMatUsu)
        Set L = LstUsuarios.ListItems.Add(, , pMatUsu(i - 1, 0))
...
Call oAcceso.CargaControles(gsDominio, MatUser, MatGrupo, R, rsOpe, gsCodUser)
...
Private Sub LstUsuarios_Click()
    If LstUsuarios.ListItems.Count > 0 Then
        ...
        'Para Acceso a Operaciones
        If TVOperacion.Nodes.Count > 1 Then
            Call CargaAccesos(TVOperacion.Nodes(1)) 'AMDO 20130717
            TVOperacion.Nodes(1).Expanded = True
            TVOperacion.Nodes(1).Checked = TodosMarcados(TVOperacion.Nodes(1).Child)
        End If
    End If
        ...
        If UCase(Node.Text) <> "MENU" Then
            If bChecked = True Then
                If Not Node.Checked Then
                    ...
                    Call oPista.InsertarPista(gsOpeCod, lsMovNro, gsCodPersUser, GetMaquinaUsuario, "1", "Otorga Permiso a " & IIf(sTipoUsu = "1", "USUARIO " & UCase(sUsuario), UCase(sUsuario)) & " la operación " & Replace(Node.Text, "&", "") & " cuya ruta es " & Replace(CStr(Node.FullPath), "&", ""))
            ...
            Else
                ...
                Set oNContFunciones = New NContFunciones
                lsMovNro = oNContFunciones.GeneraMovNro(gdFecSis, gsCodAge, gsCodUser)
                Call oPista.InsertarPista(gsOpeCod, lsMovNro, gsCodPersUser, GetMaquinaUsuario, "3", "Denegar Permiso a " & IIf(sTipoUsu = "1", "USUARIO " & UCase(sUsuario), UCase(sUsuario)) & " la operación " & Replace(Node.Text, "&", "") & " cuya ruta es " & Replace(CStr(Node.FullPath), "&", ""))
                Set oNContFunciones = Nothing
                '***Fin Agregado por ELRO**********************************
                Node.Checked = False
            End If
...
'***Fin Agregado por ELRO**********************************
        ...
        If UCase(Node.Text) <> "MENU" Then
            ...
            Else
                ...
                Set oNContFunciones = Nothing
                '***Fin Agregado por ELRO**********************************
'                Call oAcceso.DenegarAcceso(sUsuario, Mid(Node.Tag, 1, 3) & "0000000000", sTipoUsu) 'AMDO20130717
...
'        End If
'        TVMenu.Nodes(1).Checked = TodosMarcados(TVMenu.Nodes(1).Child) 'AMDO20130717
        Screen.MousePointer = 0
End Sub
