...
Begin VB.Form frmLogVehiculoMnt 
   ...
   ClientTop       =   2895
   ClientWidth     =   10110
   Icon            =   "frmLogVehiculoMnt.frx":0000
   LinkTopic       =   "Form2"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   4575
   ...
   End
End
Attribute VB_Name = "frmLogVehiculoMnt"
...
If frmLogVehiculoLista.vpSeleccion Then
   If oConn.AbreConexion Then
      Set rs = oConn.CargaRecordSet("select nVehiculoCod from LogVehiculo where cBSCod = '" & Trim(frmLogVehiculoLista.vpCodigo) & "' and cBSSerie='" & Trim(frmLogVehiculoLista.vpSerie) & "'")
      If Not rs.EOF Then
...
If oConn.AbreConexion Then
   Set rs = oConn.CargaRecordSet("select nVehiculoCod from LogVehiculo where cBSCod = '" & Trim(txtBSCod.Text) & "' and cBSSerie='" & Trim(txtSerie.Text) & "'")
   If Not rs.EOF Then
      MsgBox "Ya se ha registrado el vehículo indicado..." + Space(10), vbInformation, "Aviso"
      Exit Sub
   End If
...
Call DV.InsertaRegVehiculo(txtBSCod.Text, txtSerie, nTipo, Me.TxtModelo, Me.CboAño.Text, 1, nCombustible, Me.TxtNroMotor, Me.TxtPlaca + "-" + Me.txtPlacaNro, nTipVehiculo, nMarcaV, nColoV, txtFechaIni, txtFechaFin, gdFecSis, Right(gsCodAge, 2), gsCodUser)
