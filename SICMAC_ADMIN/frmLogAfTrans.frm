...
Private Sub cmdProcesar_Click()
    ...
    While Not rs.EOF
        If rs!bSerie Then
            Set rsSerie = oALmacen.GetSerieMov(rs!cBSCod, rs!cDocNro, 42)   'rs!nMovNro)
        End If
        ...
        For lnI = 1 To rs!nMovCant
            If rs!bSerie = 0 Then
                lsSerie = Trim(Str(Year(gdFecSis))) & "-" & Format(CInt(oALmacen.GetCorrelaSerie(rs!cBSCod)) + 1, "00000000")
...
Private Sub cmdTrasferirActivoFijo_Click()
    ...
    oALmacen.TransferenciaActivoFijo Year(gdFecSis), CDate(Me.mskFecIni.Text), CDate(Me.mskFecFin.Text)
...
Private Sub Salidas()
    ...
    While Not rs.EOF
        If rs!bSerie Then
            Set rsSerie = oALmacen.GetSerieMov(rs!cBSCod, rs!cDocNro, TpoDocAlmacenGuiadeRemision)
            lbBan = True
        End If
        ...
        For lnI = 1 To rs!nMovCant
            If rs!bSerie = 0 Or (rsSerie.EOF And rs.RecordCount > rs.Bookmark) Then
                lsSerie = Trim(Str(Year(gdFecSis))) & "-" & Format(CInt(oALmacen.GetCorrelaSerie(rs!cBSCod)) + 1, "00000000")
                oALmacen.InsertMovBsControl Year(gdFecSis), rs!nMovNro, rs!cBSCod, lsSerie, Format(rs!nMovImporte / rs!nMovCant, "#.00"), Format((rs!nMovImporte / rs!nMovCant) * gnIGV, "#.00"), 0, 0, Format(CDate(rs!dDocFecha), gsFormatoFecha), rs!cAreaCod, rs!cAgeCod, 60, 0, rs!cBSDescripcion, 1, 0, 1, "", "", "", rs!cCtaContCod, "", False
                lnIGVAcum = lnIGVAcum + Format(rs!nMovImporte / rs!nMovCant, "#.00")
                lnValorAcum = lnValorAcum + Format((rs!nMovImporte / rs!nMovCant) * gnIGV, "#.00")
            Else
                'If lbBan And rsSerie.BOF And rsSerie.EOF Then
                '    oAlmacen.SetSerieMov rs!cBSCod, rs!cDocNro, TpoDocAlmacenGuiadeRemision, rs!nMovCant, rs!nMovNro, rs!nMovItem, gdFecSis
                '    Set rsSerie = oAlmacen.GetSerieMov(rs!cBSCod, rs!cDocNro, TpoDocAlmacenGuiadeRemision)   'rs!nMovNro)
                '    lbBan = False
                'End If
                ...
                If rsSerie!nValor = 0 Then
                    oALmacen.InsertMovBsControl Year(gdFecSis), rs!nMovNro, rs!cBSCod, lsSerie, Format(rs!nMovImporte / rs!nMovCant, "#.00"), Format((rs!nMovImporte / rs!nMovCant) * gnIGV, "#.00"), 0, 0, Format(CDate(rs!dDocFecha), gsFormatoFecha), rs!cAreaCod, rs!cAgeCod, 60, 0, rs!cBSDescripcion, 1, 0, 1, "", "", lsProveedor, rs!cCtaContCod, lsFactura, False
                Else
                    oALmacen.InsertMovBsControl Year(gdFecSis), rs!nMovNro, rs!cBSCod, lsSerie, rsSerie!nValor, rsSerie!nIGV, 0, 0, Format(CDate(rs!dDocFecha), gsFormatoFecha), rs!cAreaCod, rs!cAgeCod, 60, 0, rs!cBSDescripcion, 1, 0, 1, "", "", lsProveedor, rs!cCtaContCod, lsFactura, False
                End If
